        ...
        if len(candi_list):
            ...
            for a in candi_list:
                if (
                    cmd.select(
                        "termi",
                        "(%s and (not trunk) and (neighbor id %d))" % (res_sele, a[1]),
                    )
                    > 0
                ):
                    ...
                    cmd.edit(
                        "(%s and trunk and (neighbor id %d))" % (res_sele, a[1]),
                    ...
                    if "pkfrag2" in cmd.get_names("selections"):
                        ...
                        cmd.select(
                            "trunk", "(trunk or (%s and id %d))" % (res_sele, a[1])
                        )
                else:
            ...
            if len(dihe_list):
                ...
                cmd.select(
                    "termi",
                    "(%s and (not trunk) and (neighbor id %d))" % (res_sele, third_id),
                )
                pymol.stored.list = []
    ...
    for a in id_list:
        ...
        if len(lst):
            ...
            for a in lst:
                ...
                for b in a:
                    cmd.iterate(
                        "(%s and id %d)" % (res_sele, b), "stored.names.append(name)"
                    )
                name_list.append(tuple(pymol.stored.names))
            res_dict = {}
            for a in name_list:
                dihe = cmd.get_dihedral(
                    "(%s and name %s)" % (res_sele, a[0]),
