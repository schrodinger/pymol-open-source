...
def load(self, fname):
    ...
    while 1:
        ...
        l = "  " + l
        a4 = string.strip(l[0:4])
        self.vdw[a4] = [float(l[4:20]), float(l[20:37]), string.strip(l[37:])]
    ...
    while 1:
        ...
        if not l:
            break
        if l[0:6] == "TINKER":
            self.extra[string.strip(l[6:12])] = [int(l[12:18]), int(l[18:24])]
...
os.system(
    "sed 's/CT/D4/g;s/C[DM]/D3/g;s/C[\*AKQRVW]/DA/g;"
    + "s/-C /-DJ/g;s/C -/DJ-/g;s/C[BCN]/DJ/g;s/C[ZY]/D2/g;"
    + "s/C[FX]/DA/g' parm_simple.inp > tmp1.dat"
)

os.system(
    "sed 's/N[3T]/J4/g;s/-N /-J3/g;s/N -/J3-/g;s/N\*/J3/g;"
    + "s/N[A2]/J3/g;s/N[BCX]/JA/g;s/NY/J1/g' tmp1.dat > tmp2.dat"
)

os.system(
    "sed 's/H[OWSAC123P45Z]/A /g;s/-H /-A /g;s/H -/A -/g' " + "tmp2.dat > tmp3.dat"
)

os.system(
    "sed 's/O2/QN/g;s/-O /-Q1/g;s/O -/Q1-/g;s/O[HSW]/Q2/g;' " + "tmp3.dat > tmp4.dat"
)

os.system(
    "sed 's/SO/T4/g;s/SX/TA/g;s/SH/T2/g;s/-S /-T2/g;s/S -/T2-/g;' "
    + "tmp4.dat > tmp5.dat"
)

os.system(
    "sed 's/-F /-R1/g;s/F -/R1-/g;s/-I /-R4/g;s/I -/R4-/g;"
...
for a in kees:
    if len(tmp.torsion[a]) == 1:
        ...
        for b in tmp.torsion[a][1:]:
            if (
                (f1 != float(b[0][0:6]))
                or (f3 != float(b[0][14:27]))
                or (f4 != float(b[0][27:42]))
            ):
                flag = 1
                break
        ...
        if not flag:
            f2 = f2 / c
            print(
                "%-1s%4d%8.2f%13.1f%14d.         combination of %d"
                % (a, f1, f2, f3, f4, c)
            )
        else:
            flag = 0
