...
class SettingSlider(QtWidgets.QSlider):
    ...
    def setDoubleValue(self, val):
        self.setValue(int(self.maximum() * (val - self.min_val) / self.value_range))
...
def preset_default():
    ...
    update_setting("spec_direct_power", 55.0)
    update_setting("light_count", 2)
    update_setting("shininess", 55.0)
    update_setting("reflect", 0.45)
    update_setting("spec_count", -1)
    update_setting("spec_power", -1.0)
    update_setting("spec_reflect", -1.0)
    ...
    update_setting("power", 1.0)
    update_setting("reflect_power", 1.0)


def preset_metal():
    update_setting("ambient", 0.2)
    update_setting("direct", 0.0)  # diffuse
    update_setting("spec_direct", 0)
    update_setting("shininess", 51.0)  # same as spec_power
...
def preset_plastic():
    update_setting("ambient", 0.0)
    update_setting("direct", 0.2)  # diffuse
    update_setting("spec_direct", 0)
    update_setting("shininess", 32.0)  # same as spec_power
...
def preset_rubber():
    ...
    update_setting("direct", 0.2)  # diffuse
    update_setting("spec_direct", 0)
    update_setting("shininess", 10.0)  # same as spec_power
    update_setting("reflect", 0.5)  # diffuse
    update_setting("spec_count", -1)
    update_setting("spec_reflect", -1.0)
    update_setting("specular", 1)
...
def create_dialog():
    ...
    sliders = [
        ...
        ("ambient_occlusion_scale", 1.0, 50.0, None),
    ...
    for i, item in enumerate(sliders, 1):
        ...
        slider = SettingSlider(dialog, name.split()[0], min, max, res, line_edit)
