...
def run_impl(form, _self):
    ...
    if form.do_prepare.isChecked():
        ...
        get_res_charge = lambda resn: (
            (-1) if resn in ("GLU", "ASP") else (1) if resn in ("ARG", "LYS") else 0
        )
        ...
        if method == "pdb2pqr":
            ...
            _self.create(prep_name, selection)
            _self.alter(
                prep_name, "(elec_radius, partial_charge) = (vdw, formal_charge)"
            )
...
def dialog(_self=None):
    ...
    def run_finally(args):
        ...
        if QMessageBox.Yes == QMessageBox.question(
            form._dialog, "Finished", quit_msg, QMessageBox.Yes, QMessageBox.No
        ):
            form._dialog.close()
    ...
    def handle_exception(e, stdout):
        ...
        msgbox = QMessageBox(
            QMessageBox.Critical, "Error", msg, QMessageBox.Close, form._dialog
        )
        if stdout.strip():
    ...
    # hook up events
    form.input_sele.currentIndexChanged.connect(lambda: check_sele_timer.start(0))
    form.input_sele.editTextChanged.connect(lambda: check_sele_timer.start(1000))
    ...
    def _():
        fnames = getOpenFileNames(None, filter="APBS Input (*.in);;All Files (*)")[0]
        if fnames:
