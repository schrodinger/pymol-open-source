...
try:
    ...
    _osx_support._UNIVERSAL_CONFIG_VARS += (
        "CXXFLAGS",
        "LDCXXSHARED",
    )
...
def customize_compiler(compiler):
    ...
    if sys.platform == "linux" and (
        "icpc" in os.getenv("CXX", "")
        or "clang" in os.getenv("CC", "")
        or "clang" in os.getenv("LD", "")
    ):
        import re
    ...
    (cxx, ccshared, ldcxxshared) = distutils.sysconfig.get_config_vars(
    ...
    ldcxxshared = (
        os.environ.get("LDCXXSHARED", ldcxxshared or "")
        + " "
...
def compile(
    ...
    if sys.platform == "darwin" and _osx_support is not None:
        ...
        compiler_so_cxx = _osx_support.compiler_fixup(
            compiler_so_cxx, cc_args + extra_postargs
        )
    ...
    def _single_compile(obj):
        ...
        # _compile
        compiler = (
            compiler_so_cxx if self.detect_language(src) == "c++" else compiler_so
        )
        try:
    ...
    def msvc_spawn_and_write_d_file(obj, src, cmd, env, dry_run):
        ...
        if process.returncode != 0:
            raise DistutilsExecError(
                "command %r failed with exit status %d" % (cmd, process.returncode)
            )
