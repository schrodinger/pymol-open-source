...
static int *SelectorSelect(PyMOLGlobals * G, char *sele, int state, int domain, int quiet);
...
static ov_diff SelectGetNameOffset(PyMOLGlobals * G, char *name, ov_size minMatch,
                                   int ignCase)
{
    ...
    if(first_pass) {
      for(a = cNDummyAtoms; a < I->NAtom; a++) {
        ...
        if(SelectorIsMember(G, ai->selEntry, present)) {
          ...
          if((ai->protons == cAN_C) && (WordMatch(G, "CA", ai->name, true) < 0)) {
...

            {
              ...
              for(aa = a0 + 1; aa < a1; aa++) {
                ...
                if((ai->protons == cAN_C) && (WordMatch(G, "C", ai->name, true) < 0)) {
                  found_C = aa;
                }
                if((ai->protons == cAN_N) && (WordMatch(G, "N", ai->name, true) < 0)) {
                  found_N = aa;
                }
                if((ai->protons == cAN_O) && (WordMatch(G, "O", ai->name, true) < 0)) {
                  found_O = aa;
                }
              }
              ...
              if((found_C) && (found_N) && (found_O)) {
                ...
                if(!quiet) {
                  PRINTFB(G, FB_Selector, FB_Warnings)
                    ...
                    obj->Obj.Name, ai->segi, LexStr(G, ai->chain), ai->resi ENDFB(G);
                }
              }
            }
          }
          ...
          if((cc1 == sta1) || (sta1 < 0)) {
            ...
            if(cs1 && (((sta0 < 0) && (cc1 < obj0->NCSet)) ||   /* multiple states */
              ...
              if(cs0) {
                ...
                if((ci0 >= 0) && (ci1 >= 0))
                  copy3f(cs1->Coord + 3 * ci1, cs0->Coord + 3 * ci0);
              }
            }
          }
        }
...
{
  return SelectorGetTmpImpl(G, input, store, 0);
}

int SelectorGetTmpQuiet(PyMOLGlobals * G, char *input, char *store)
{
  return SelectorGetTmpImpl(G, input, store, 1);
}
/*========================================================================*/
...
static int _SelectorCreate(PyMOLGlobals * G, char *sname, char *sele,
...
int SelectorCreateWithStateDomain(PyMOLGlobals * G, char *sname, char *sele,
                                  ObjectMolecule * obj, int quiet, Multipick * mp,
  ...
  case SELE_RSIs:
    {
      ...
      if((matcher = WordMatcherNew(G, base[1].text, &options, true))) {
        ...
        for(a = cNDummyAtoms; a < I_NAtom; a++) {
          ai = i_obj[table_a->model]->AtomInfo + table_a->atom;
          if((*base_0_sele_a = WordMatcherMatchMixed(matcher, ai->resi, ai->resv)))
            c++;
          table_a++;
          base_0_sele_a++;
        }
