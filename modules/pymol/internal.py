...
def _ray_anti_spawn(thread_info, _self=cmd):
    ...
    for a in thread_info[1:]:
        t = threading.Thread(target=_cmd.ray_anti_thread, args=(_self._COb, a))
        t.setDaemon(1)
...
def _ray_hash_spawn(thread_info, _self=cmd):
    ...
    for a in thread_info[1:]:
        if a is not None:
            t = threading.Thread(target=_cmd.ray_hash_thread, args=(_self._COb, a))
            t.setDaemon(1)
            thread_list.append(t)
    for t in thread_list:
        t.start()
...
def _ray_spawn(thread_info, _self=cmd):
    ...
    thread_list = []
    for a in thread_info[1:]:
        t = threading.Thread(target=_cmd.ray_trace_thread, args=(_self._COb, a))
        t.setDaemon(1)
        thread_list.append(t)
    for t in thread_list:
        t.start()
...
def _coordset_update_spawn(thread_info, n_thread, _self=cmd):
    ...
    if len(thread_info):
        ...
        for a in range(1, n_thread):
            t = threading.Thread(
                target=_coordset_update_thread, args=(list_lock, thread_info)
            )
            t.setDaemon(1)
            thread_list.append(t)
        for t in thread_list:
...
def _object_update_spawn(thread_info, n_thread, _self=cmd):
    ...
    if len(thread_info):
        ...
        for a in range(1, n_thread):
            t = threading.Thread(
                target=_object_update_thread, args=(list_lock, thread_info)
            )
            t.setDaemon(1)
            thread_list.append(t)
        for t in thread_list:
...
def file_read(finfo, _self=cmd):
    ...
    try:
        if not is_string(finfo):
            ...
            req = urllib2.Request(
                finfo, headers={"User-Agent": "PyMOL/" + _self.get_version()[0]}
            )
