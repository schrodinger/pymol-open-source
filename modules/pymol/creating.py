...
if True:
    ...
    reflection_format_dict = {"cns": 1, "mtz": 2, "cif": 3}
    ...
    def group(name, members="", action="auto", quiet=1, _self=cmd):
        ...
        """
        action = group_action_dict[
            group_action_sc.auto_err(str(action), "group action")
        ]
        if name == "all":
            name = "*"
        if action == 6:  # auto
        ...
        with _self.lockcm:
            return _cmd.group(
                _self._COb, str(name), str(members), int(action), int(quiet)
            )
    ...
    def map_generate(
        ...
        try:
            ...
            for dataset in list(mtzFile.datasets.values()):
                if (
                    not datasetName or dataset["name"] == datasetName
                ) and ampColName in dataset["cols"]:
                    break
            else:
    ...
    def ramp_new(
        ...
        if is_list(color):
            for a in color:
                if not is_list(a):
                    new_color.append(
                        list(_self.get_color_tuple(a, 4))
                    )  # incl negative RGB special colors
                else:
                    new_color.append(a)
        elif is_string(color):
            new_color = ramp_spectrum_dict[
                ramp_spectrum_sc.auto_err(str(color), "ramp color spectrum")
            ]
        else:
            new_color = int(color)
        try:
    ...
    def set_raw_alignment(name, raw, guide="", state=1, quiet=1, _self=cmd):
        ...
        """
        with _self.lockcm:
            return _cmd.set_raw_alignment(
                name, raw, guide, int(state) - 1, int(quiet), _self._COb
            )
    ...
    def slice_new(name, map, state=1, source_state=0, _self=cmd):
        ...
        try:
            _self.lock(_self)
            r = _cmd.slice_new(
                _self._COb, str(name), str(map), int(state) - 1, int(source_state) - 1
            )
        finally:
            _self.unlock(r, _self)
        if _self._raising(r, _self):
            ...
            if la and int(origin):
                ...
                for c in range(0, 3):
                    ...
                    for a in model.atom:
                        a.coord[c] += mean_c
            r = _self.load_model(
                model, str(object), quiet=quiet, zoom=zoom, _self=_self
            )
        except IOError:
    ...
    def pseudoatom(
        ...
        if len(color):
            ...
            color = -1  # default
        selection = selector.process(selection)
        mode = pseudoatom_mode_dict[
            pseudoatom_mode_sc.auto_err(str(mode), "pseudoatom mode")
        ]

        (name, resn, resi, chain, segi, elem, label) = list(
            map(unquote, (name, resn, resi, chain, segi, elem, label))
        )
        #
        try:
            _self.lock(_self)
            if pos is not None:
                ...
                pos = (float(pos[0]), float(pos[1]), float(pos[2]))  # tuple-ize
    ...
    def join_states(name, selection="all", mode=2, zoom=0, quiet=1, _self=cmd):
        ...
        for i, model in enumerate(models):
            ...
            if mode == 2 and i > 0:
                _self.remove(
                    "?%s & ! ((alt A+) & (?%s in ?%s))" % (name, name, msel_name)
                )
