...
if True:
    ...
    def cealign(
        ...
        *,
        ...
        try:
            ...
            # call the C function
            r = _cmd.cealign(
                _self._COb, sel1, sel2, float(d0), float(d1), int(window), int(gap_max)
            )
            ...
            if object is not None:
                ...
                if len(obj1) > 1 or len(obj2) > 1:
                    print(
                        " CEalign-Error: selection spans multiple"
                ...
                _self.select_list(
                    tmp1, obj1[0], [ids1[j] for i in i1 for j in range(i, i + window)]
                )
                _self.select_list(
                    tmp2, obj2[0], [ids2[j] for i in i2 for j in range(i, i + window)]
                )
    ...
    def alignto(
        target="", method="cealign", selection="", quiet=1, *, _self=cmd, **kwargs
    ):
        """
        DESCRIPTION
    ...
    def intra_fit(selection, state=1, quiet=1, mix=0, *, pbc=1, _self=cmd):
        ...
        with _self.lockcm:
            r = _cmd.intrafit(
                _self._COb, selection, int(state) - 1, 2, int(quiet), int(mix), int(pbc)
            )
            ...
            for a in r:
                if a >= 0.0:
                    if mix:
                        ...
                        print(
                            " cmd.intra_fit: %5.3f in state %d vs state %d"
                            % (a, st, state)
                        )
                st = st + 1
        if _self._raising(r, _self):
    ...
    def intra_rms(selection, state=0, quiet=1, *, _self=cmd):
        ...
        if not isinstance(r, list):
            ...
            for a in r:
                if a >= 0.0:
                    print(
                        " cmd.intra_rms: %5.3f in state %d vs state %d" % (a, st, state)
                    )
                st = st + 1
        if _self._raising(r, _self):
            raise pymol.CmdException
    ...
    def intra_rms_cur(selection, state=0, quiet=1, *, _self=cmd):
        ...
        if not isinstance(r, list):
            ...
            for a in r:
                if a >= 0.0:
                    print(
                        " cmd.intra_rms_cur: %5.3f in state %d vs state %d"
                        % (a, st, state)
                    )
                st = st + 1
        if _self._raising(r, _self):
