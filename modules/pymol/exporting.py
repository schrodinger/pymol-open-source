...
if True:
    ...
    def cache(action="optimize", scenes="", state=-1, quiet=1, *, _self=cmd):
        ...
        if action == 0:  # enable
            ...
            if not quiet:
                print(
                    " cache: optimization complete (~%0.1f MB)."
                    % (usage * 4 / 1000000.0)
                )
        else:
    ...
    def get_pdbstr(
        selection="all", state=-1, ref="", ref_state=-1, quiet=1, *, _self=cmd
    ):
        """
        DESCRIPTION
        ...
        def convert_settings(settings_list):
            ...
            if version < 1.76:
                changed += [
                    ...
                    (93, 6.0, -1.0),  # cartoon_loop_quality
                    (102, 10.0, -1.0),  # cartoon_oval_quality
                    (104, 9.0, -1.0),  # cartoon_tube_quality
                    (378, 11.0, -1.0),  # cartoon_putty_quality
                ]
            if version < 1.5:
                changed += [
                    (421, -1, 9),  # sphere_mode
    ...
    def get_session(
        ...
        *,
        ...
        if cache:
            ...
            if cache_opt == 1 or (
                cache_opt != 0 and _self.get_setting_int("cache_mode")
            ):
    ...
    def _unit2px(value, dpi, unit=""):
        ...
        if unit not in upi:
            raise pymol.CmdException(
                "unknown unit, supported units are: " + ", ".join(upi)
            )

        if dpi < 1:
            raise pymol.CmdException(
                'dpi > 0 required with unit "%s" '
    ...
    def multisave(
        filename, pattern="all", state=-1, append=0, format="", quiet=1, *, _self=cmd
    ):
        """
        DESCRIPTION
    ...
    def assign_atom_types(selection, format="mol2", state=1, quiet=1, *, _self=cmd):
        ...
        with _self.lockcm:
            ...
            _cmd.assign_atom_types(_self._COb, selection, fmt, int(state - 1), quiet)
    ...
    def multifilenamegen(filename, selection, state, _self=cmd):
        ...
        fmt_keys = [
            v[1] for v in string.Formatter().parse(filename) if v[1] is not None
        ]
        ...
        for num, (oname, osele, ostate) in enumerate(odata, 1):
            ...
            fname = filename.format(
                oname, ostate, name=oname, state=ostate, num=num, title=title
            )
    ...
    def save(
        ...
        *,
        ...
        if format in savefunctions:
            ...
            func_type4[format](
                _self.get_model(selection, state, ref, ref_state), filename
            )
            r = DEFAULT_SUCCESS
        else:
    ...
    def get_alnstr(selection, state=-1, quiet=1, *, _self=cmd):
        with _self.lockcm:
            return _cmd.get_seq_align_str(
                _self._COb, str(selection), int(state) - 1, 0, int(quiet)
            )
    ...
    def get_ccp4str(name, state=1, quiet=1, format="ccp4", *, _self=cmd):
        ...
        with _self.lockcm:
            return _cmd.get_ccp4str(
                _self._COb, str(name), int(state) - 1, int(quiet), ftype
            )
