...
from .cmd import DEFAULT_ERROR, DEFAULT_SUCCESS, _raising, is_list, is_string, is_error
...
def attach_amino_acid(
    selection, amino_acid, center=0, animate=-1, object="", hydro=-1, ss=-1, _self=cmd
):
    """
    ARGUMENTS
    ...
    if (
        selection not in _self.get_names("all")
        ...
        amino_acid in ["nhh", "nme"]
        and _self.select(tmp_connect, "(%s) & elem C" % selection) != 1
    ):
        print(
            "Error: invalid connection point: must be C for residue '%s'" % (amino_acid)
        )
        _self.delete(tmp_wild)
        raise QuietException
    elif (
        amino_acid in ["ace"]
        and _self.select(tmp_connect, "(%s) & elem N" % selection) != 1
    ):
        print(
            "Error: invalid connection point: must be N for residue '%s'" % (amino_acid)
        )
        _self.delete(tmp_wild)
        raise QuietException
    else:
        ...
        if _self.count_atoms("elem N", domain=tmp_connect):
            ...
            if (_self.select(tmp1, "?pk1", domain=tmp_domain) == 1) and (
                ...
                if (
                    _self.select(tmp3, "(name CA,CH3 & nbr. ?pk1)", domain=tmp_domain)
                    == 1
                ) and (
                    _self.select(tmp4, "(name CA,CH3 & nbr. ?pk2)", domain=tmp_domain)
                    == 1
                ):
                ...
                if ss:
                    if amino_acid[0:3] != "pro":
                        if (
                            _self.select(
                                tmp4,
                                "(!(resn PRO) & name C  & nbr. (name CA & nbr. "
                                + tmp2
                                + "))",
                                domain=tmp_domain,
                            )
                            ...
                            _self.set_dihedral(  # PHI
                                tmp4, tmp3, tmp2, tmp1, phi  # C  # CA  # N  # C
                            )

                    if (
                        _self.select(
                            tmp4,
                            "(name N & nbr. (name CA & nbr. " + tmp1 + "))",
                            domain=tmp_domain,
                        )
                        == 1
                    ) and (
                        _self.select(
                            tmp3, "(name CA & nbr. " + tmp1 + ")", domain=tmp_domain
                        )
                        == 1
                    ):
                        _self.set_dihedral(  # PSI (n-1)
                            tmp2, tmp1, tmp3, tmp4, psi  # N  # C  # CA  # N
                        )

            sele = "(name N & (byres nbr. %s) &! (byres %s))" % (
                tmp_connect,
                tmp_connect,
            )
            ...
            if (_self.select(tmp1, "?pk1", domain=tmp_domain) == 1) and (
                ...
                if ss:
                    if hydro and amino_acid[0:3] == "nhh":  # fix amide hydrogens
                        if (
                            _self.select(
                                tmp3, "(name H1 & nbr. " + tmp1 + ")", domain=tmp_domain
                            )
                            == 1
                        ) and (
                            _self.select(
                                tmp4, "(name O & nbr. " + tmp2 + ")", domain=tmp_domain
                            )
                            == 1
                        ):
                            _self.set_dihedral(
                                tmp4, tmp2, tmp1, tmp3, 180  # O  # C  # N  # H1
                            )
                    if amino_acid[0:3] != "pro":
                        if (
                            _self.select(
                            ...
                            _self.select(
                                tmp4,
                                "(name C & nbr. (name CA & nbr. " + tmp1 + "))",
                                domain=tmp_domain,
                            )
                            == 1
                        ):
                            _self.set_dihedral(  # PHI
                                tmp2, tmp1, tmp3, tmp4, phi  # C  # N  # CA  # C
                            )
                    if (
                        _self.select(
                            tmp3, "(name CA & nbr. " + tmp2 + ")", domain=tmp_domain
                        )
                        == 1
                    ) and (
                        _self.select(
                            tmp4,
                            "(name N & nbr. (name CA & nbr. " + tmp2 + "))",
                            domain=tmp_domain,
                        )
                        == 1
                    ):
                        _self.set_dihedral(  # PSI (n-1)
                            tmp4, tmp3, tmp2, tmp1, psi  # N  # CA  # C  # N
                        )
            sele = "(name C & (byres nbr. %s) & !(byres %s))" % (
                tmp_connect,
                tmp_connect,
            )
            if _self.select(tmp1, sele, domain=tmp_domain):
        ...
        if code is not None:
            while len(sequence):
                while len(sequence) and "/" in sequence[-1]:
                    ...
                    if len(part) > 3:
                        segi = part[-4]
                if len(sequence) and not _self.count_atoms(
                    "?pk1"
                ):  # new polymer segment
                    ...
                    _self.edit(
                        name + " in " + tmp_sel
                    )  # set the editor's pk1 selection
...
def fab(
    ...
    **kwargs,
    ...
    if async_ < 1:
        r = _fab(
            input, name, mode, resi, chain, segi, state, dir, hydro, ss, quiet, _self
        )
    else:
        fab_thread = threading.Thread(
            target=_fab,
            args=(
                input,
                name,
