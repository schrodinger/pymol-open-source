...
class _PymolHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def process_request(self):
        ...
        try:
            ...
            if host[0:6] != "127.0.":
                self.send_error(
                    403, "Only localhost requests are allowed (not: %s)" % host
                )
            else:
    ...
    def send_json_error(self, code, message):
        if self.callback is not None:
            ...
            self.wfile_write(
                "%s(%s)" % (self.callback, self.wrap_return(message, "ERROR"))
            )
        else:
            accept_mime = self.headers.get("Accept")
            if accept_mime in _json_mime_types:
            ...
            for block in blocks:
                ...
                if fn is not None:
                    ...
                    try:
                        ...
                        self.send_exception_json(
                            500,
                            [
                                "Exception in: %s" % block[0],
                                "Args: " + str(args),
                                "Kwds: " + str(kwds),
                            ],
                        )
                    ...
                    self.send_json_error(500, ["Method not found:", str(block)])
                    return
    ...
    def send_doc(self):
        ...
        if ".." in path_list:  # prevent access to parent directories
            ...
            try:
                full_path = os.path.join(*[self.server.pymol_root] + list(path_list))
...
class PymolHttpd:
    ...
    def __init__(
        self, port=8080, root=None, logging=1, wrap_natives=0, self_cmd=None, headers=()
    ):
        """
        ...
        self.server = BaseHTTPServer.HTTPServer(
            ("", self.port), _PymolHTTPRequestHandler
        )
    ...
    def start(self):
        print(" PyMOL-HTTPd: serving requests on http://localhost:%d" % self.port)
