...
def addmenuitem(label, command=None, menuName="Plugin"):
    ...
    if pmgapp is not None:
        for i in range(1, len(labels2)):
            try:
                pmgapp.menuBar.addcascademenu(
                    labels2[i - 1], labels2[i], label=labels1[i]
                )
            except ValueError:
                pass
        if labels1[-1] == "-":
            ...
            pmgapp.menuBar.addmenuitem(
                labels2[-1], "command", label=labels1[-1], command=command
            )
...
class PluginInfo(object):
    ...
    def get_docstring(self):
        ...
        try:
            c = compile(
                b"".join(open(self.filename, "rb")), "x", "exec", dont_inherit=True
            )
            s = c.co_consts[0]
            if cmd.is_string(s):
                return s
    ...
    def load(self, pmgapp=None, force=0):
        ...
        try:
            ...
            if verbose and pymol.invocation.options.show_splash:
                print(
                    ' Plugin "%s" loaded in %.2f seconds' % (self.name, self.loadtime)
                )
        except QtNotAvailableError:
            colorprinting.warning(
                "Plugin '%s' only available with PyQt GUI." % (self.name,)
            )
        except:
            e = sys.exc_info()[1]
            if verbose:
            ...
            colorprinting.warning(
                "Unable to initialize plugin '%s' (%s)." % (self.name, self.mod_name)
            )
            return False
    ...
    def uninstall(self, parent=None):
        ...
        try:
            ...
            showinfo(
                "Error",
                'Could not delete files for plugin "%s".' % (self.name),
                parent=parent,
            )
            return False
        ...
        showinfo(
            "Info",
            'Plugin "%s" successfully removed. Please restart PyMOL.' % (self.name),
            parent=parent,
        )
        return True
