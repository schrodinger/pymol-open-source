...
def extract_zipfile(ofile, ext):
    ...
    for f in namelist:
        ...
        for part in f.split(
            "/"
        ):  # even on windows this is a forward slash (not os.sep)
            if part != "":
                x = x.setdefault(part, {})
    if len(namedict) == 0:
...
def get_plugdir(parent=None):
    ...
    if "pmg_qt.mimic_tk" in sys.modules:
        ...
        value, result = QtWidgets.QInputDialog.getItem(
            None,
            "Select plugin directory",
            "In which directory should the plugin be installed?",
            plugdirs,
        )
...
def installPluginFromFile(ofile, parent=None, plugdir=None):
    ...
    if not is_writable(plugdir):
        ...
        if not os.path.exists(user_plugdir):
            try:
                ...
                showinfo(
                    "Error", "Could not create user plugin directory", parent=parent
                )
                return
    ...
    if info.load(force=1):
        ...
        showinfo(
            "Error",
            'Plugin "%s" has been installed but initialization failed.' % name,
            parent=parent,
        )
