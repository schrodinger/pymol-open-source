...
def confirm_network_access():
    ...
    if self.ok < 0:
        tkMessageBox.showinfo("Info", "Network download has been disabled, sorry!")
        return False
    if self.ok > 0:
        return True
    self.ok = tkMessageBox.askyesno(
        "Confirm",
...
def plugin_info_dialog(parent, info):
    ...
    def add_line(label, text):
        ...
        Tkinter.Label(grid, text=label + ":", bg=bg).grid(
    ...
    if not info.is_temporary:
        if not info.loaded:
            Tkinter.Label(
                grid,
                text="more information might be available after plugin is loaded",
                bg=bg_notice,
                padx=10,
                pady=10,
            ).grid(columnspan=2, sticky="nesw")
    ...
    if docstring is not None:
        ...
        Tkinter.Label(
            grid, text="no documentation available", bg=bg_notice, padx=10, pady=10
        ).grid(columnspan=2, sticky="nesw")
...
class PluginManager(Pmw.MegaToplevel):
    ...
    def page_installed(self, notebook):
        ...
        Tkinter.Checkbutton(
            f_filter,
            text="startup",
            variable=f_installed.v_fstartup,
            command=filter_command,
        ).pack(side="right")
        Tkinter.Checkbutton(
            f_filter,
            text="loaded",
            variable=f_installed.v_floaded,
            command=filter_command,
        ).pack(side="right")
        e_filter = Pmw.EntryField(
            f_filter, labelpos="w", label_text="Filter:", modifiedcommand=filter_command
        )
        e_filter.pack(side="left", expand=1, fill="x")

        # enable/disable all items
        f_all = Tkinter.Frame(page)
        Tkinter.Button(f_all, text="startup all", command=f_installed.startup_all).pack(
            side="left"
        )
        Tkinter.Button(
            f_all, text="startup none", command=f_installed.startup_none
        ).pack(side="left")
    ...
    def page_install_new(self, notebook):
        ...
        if confirm_network_access.ok < 0:
            Tkinter.Label(page, text="Network access has been disabled").pack(expand=1)
            return
        ...
        pane_left = pw.add("left", min=0.2)
        pane_right = pw.add("right", min=0.2, max=0.5)
            ...
            try:
                ...
                if ext in zip_extensions:
                    ...
                    filename = os.path.join(
                        os.path.join(tmpdir, *dirnames), "__init__.py"
                    )
        ...
        def slb_left_add():
            url = tkSimpleDialog.askstring(
                "Repository URL", "Please enter Repository URL", parent=self.interior()
            )
            if not url:
                return
    ...
    def page_settings(self, notebook):
        ...
        slb_path = Pmw.ScrolledListBox(
            w.interior(), items=get_startup_path(True), listbox_height=4
        )
        if slb_path.get(0):
            slb_path.setvalue(
                [slb_path.get(0)]
        ...
        for row, key in enumerate(preferences):
            ...
            if len(value) > 100:
                value = value[:97] + "..."
            Tkinter.Label(w, text=key).grid(
                row=row, column=0, sticky="nw", padx=3, pady=2
            )
            e = Tkinter.Entry(w)
            e.insert(0, value)
            e.config(state="readonly")
...
class InstalledPluginsWidget(Pmw.ScrolledFrame):
    ...
    def __init__(self, parent):
        self.super = self.__class__.__bases__[0]
        self.super.__init__(self, parent, vscrollmode="static", horizflex="expand")
...
class PluginWidget(Tkinter.Frame):
    ...
    def __init__(self, parent, info):
        ...
        self.super = self.__class__.__bases__[0]
        self.super.__init__(
            self, parent, borderwidth=1, padx=5, pady=5, relief=Tkinter.SUNKEN
        )
        ...
        self.w_startup = Tkinter.Checkbutton(
            master,
            text="Load on startup",
            variable=self.v_startup,
            command=self.c_startup,
        )
        ...
        w_uninstall = Tkinter.Button(
            master, text="Uninstall", command=self.plugin_remove
        )
    ...
    def status_update(self):
        ...
        if self.info.loaded:
            ...
            # add 'Settings' button
            if hasattr(self.info.module, "settings_dialog") and not len(
                self.f_settings.winfo_children()
            ):
                Tkinter.Button(
                    self.f_settings,
                    text="Settings",
                    command=self.info.module.settings_dialog,
                ).pack()
