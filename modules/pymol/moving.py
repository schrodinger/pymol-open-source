...
if True:
    ...
    def mview(
        ...
        try:
            ...
            if not freeze and (
                (auto > 0)
                or (
                    (auto < 0) and (_self.get_setting_int("movie_auto_interpolate") > 0)
                )
            ):
    ...
    def mpng(
        ...
        if (
            mode == MODE_RAY
            or mode == MODE_DEFAULT
            and _self.get_setting_boolean("ray_trace_frames")
        ):
            return func()
    ...
    def mmove(target, source=0, count=-1, freeze=0, object="", quiet=1, _self=cmd):
        ...
        try:
            _self.lock(_self)
            r = _cmd.mmodify(
                _self._COb, 2, source, count, target, object, freeze, quiet
            )
        finally:
            _self.unlock(r, _self)
        if _self._raising(r, _self):
    ...
    def mcopy(target, source=0, count=-1, freeze=0, object="", quiet=1, _self=cmd):
        ...
        try:
            _self.lock(_self)
            r = _cmd.mmodify(
                _self._COb, 3, source, count, target, object, freeze, quiet
            )
        finally:
            _self.unlock(r, _self)
        if _self._raising(r, _self):
    ...
    def mdelete(count=-1, frame=0, freeze=0, object="", quiet=1, _self=cmd):
        ...
        try:
            _self.lock(_self)
            r = _cmd.mmodify(_self._COb, -1, frame, count, 0, object, freeze, quiet)
        finally:
            _self.unlock(r, _self)
        if _self._raising(r, _self):
            raise pymol.CmdException
