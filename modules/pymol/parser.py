...
if True:
                ...
                if s.strip() == layer.embed_sentinel:
                    ...
                    if etn == 0:  # embedded data
                        colorprinting.parrot(
                            " Embed: read %d lines." % (len(layer.embed_list))
                        )
                        ...
                        colorprinting.parrot(
                            " Skip: skipped %d lines." % (layer.embed_line)
                        )
                        layer.embed_sentinel = None
                else:
                    etn = layer.embed_type
                    ...
                    if str(layer.com1[-1]) == "\\":
                        ...
                        if lin:
                            ...
                            if com[0:1] == "/":
                                ...
                                lin > 1
                                and layer.input[-1:][0].split(" ", 1)[0] in py_delims
                            ):
                                self.exec_python(layer.com2, secure)
                            else:
                                ...
                                if com in self.cmd.keyword:
                                    ...
                                    if layer.kw[4] >= parsing.NO_CHECK:
                                        ...
                                        if (
                                            ...
                                            if not secure:
                                                ...
                                                raise SecurityException(
                                                    "Python expressions disallowed in this file"
                                                )
                                        if secure and (layer.kw[4] == parsing.SECURE):
                                            layer.next = []
                                            raise SecurityException(
                                                "Command disallowed in this file"
                                            )
                                        else:
                                        ...
                                        if (
                                            layer.kw[4] == parsing.MOVIE
                                        ):  # copy literal single line, no breaks
                                            layer.next = []
                                            if not secure:
                                                layer.input = layer.com1.split(" ", 1)
                                            else:
                                                raise SecurityException(
                                                    "Movie commands disallowed in this file"
                                                )
                                        ...
                                        if (
                                            ...
                                            if layer.kw[4] < parsing.RUN:
                                                ...
                                                layer.next = []
                                                if (
                                                    secure or self.nest == 0
                                                ):  # only legal on top level and p1m files
                                                    l = len(layer.args)
                                                    if l > 0:
                                                        key = layer.args[0]
                                                    else:
                                                ...
                                                print(
                                                    "Error: unknown keyword mode: "
                                                    + str(layer.kw[4])
                                                )
                                                raise QuietException
                                        else:
                                            print(
                                                "Error: invalid arguments for %s command."
                                                % com
                                            )
                                #
                                    ...
                                    if layer.input[0][0] == "@":
                                        ...
                                        if re.search(r"\.py$|\.pym$", path) is not None:
                                            if self.cmd._feedback(
                                                fb_module.parser, fb_mask.warnings
                                            ):
                                                print(
                                        ...
                                        while 1:
                                            ...
                                            if len(tmp_cmd):
                                                ...
                                                elif tmp_cmd[0] == "_" and tmp_cmd[
                                                    1:2
                                                ] in [
                                                    " ",
                                                    "",
                                                ]:  # "_ " remove echo suppression signal
                                                    inp_cmd = inp_cmd[2:]
                                            ...
                                            if pp_result is None:  # RECURSION
                                                ...
                                                if self.cmd.get_setting_boolean(
                                                    "stop_on_exceptions"
                                                ):
                                                    p_result = (
                                                        0  # signal an error occurred
                                                    )
                                                    colorprinting.error(
                                                        "PyMOL: stopped on exception."
                                                    )
                                                    break
                                        self.nest = self.nest - 1
                                        layer = self.layer[self.nest]
            ...
            if not (" " in st or "@" in st):
                ...
                if self.cmd.is_string(full):
                    try:
                        if count < len(self.cmd.auto_arg):
                            if (
                                full in self.cmd.auto_arg[count]
                ...
                if not flag:  # otherwise fallback onto filename completion
                    ...
                    if not flist and st3.startswith("$"):
                        flist = [
                            "$" + var for var in os.environ if var.startswith(st3[1:])
                        ]
    ...
    def new_parse_closure(
        self_cmd,
    ):  # create parser and return an instance-specific parse function closure
        try:
            p = Parser(self_cmd)
        except:
            traceback.print_exc()
    ...
    def new_complete_closure(
        self_cmd,
    ):  # return an instance-specific complete function closure
