    ...
    def parseFile(self):
        if self.checkFile():
            try:
                ...
                print(
                    "Error-CIFReader: Couldn't read '%s' for input." % (self.filename)
                )
...
class MTZHeader(baseHeader):
    ...
    def format_cols(self, colType=None):
        ...
        for key in self.datasets:
            ...
            self.cols.extend(["/".join(x) for x in zip(curCryst, datName, c)])
            ...
            while not (curLine.startswith("END")):
                ...
                try:
                    if field.startswith(H["VERS"]):
                        ...
                        self.space_group = space_group_map.get(
                        ...
                        self.reso_min = math.sqrt(1.0 / float(self.reso_min))
                        self.reso_max = math.sqrt(1.0 / float(self.reso_max))
