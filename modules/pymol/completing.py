...
def fragments_sc():
    ...
    return cmd.Shortcut(
        [f[:-4] for f in os.listdir(chempy.path + "fragments") if f.endswith(".pkl")]
    )
...
def wizard_sc():
    ...
    names_glob = [
        name[:-3]
        for p in pymol.wizard.__path__
...
def get_auto_arg_list(self_cmd=cmd):
    ...
    aa_vol_c = [
        lambda: cmd.Shortcut(self_cmd.get_names_of_type("object:volume")),
        "volume",
        "",
    ]
    aa_ramp_c = [
        lambda: cmd.Shortcut(self_cmd.get_names_of_type("object:ramp")),
        "ramp",
        "",
    ]
    ...
    return [
        ...
        # 3rd
        {
            ...
            "pi_interactions": aa_sel_e,
            "ramp_update": [
                self_cmd.creating.ramp_spectrum_sc,
                "ramp color spectrum",
                ", ",
            ],
            "set": aa_sel_c,
