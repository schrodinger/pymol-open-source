...
if True:
    ...
    def set(name, value=1, selection="", state=0, updates=1, log=0, quiet=1, _self=cmd):
        """
        DESCRIPTION
        ...
        if log:
            name = name_dict.get(index, name)
            _self.log("", f"cmd.set({name!r},{value!r},{selection!r},{state})\n")
        with _self.lockcm:
            r = _cmd.set(
                _self._COb,
                index,
    ...
    def unset(name, selection="", state=0, updates=1, log=0, quiet=1, _self=cmd):
        ...
        with _self.lockcm:
            r = _cmd.unset(
                _self._COb, index, selection, int(state) - 1, int(quiet), int(updates)
            )
        return r

    def unset_bond(
        name, selection1, selection2=None, state=0, updates=1, log=0, quiet=1, _self=cmd
    ):
        """
        DESCRIPTION
        ...
        if is_ok(r) and (r is not None):
            if not int(quiet):
                ...
                if selection == "":
                    ...
                    print(
                        " get: %s = %s in object %s state %d"
                        % (name, r_str, selection, state)
                    )
        return r
    ...
    def get_setting_tuple_new(name, object="", state=0, _self=cmd):  # INTERNAL
        i = _get_index(name)
        with _self.lockcm:
            return _cmd.get_setting_of_type(
                _self._COb, i, str(object), int(state) - 1, cSetting_tuple
            )
    ...
    def get_setting_boolean(name, object="", state=0, _self=cmd):  # INTERNAL
        ...
        with _self.lockcm:
            return _cmd.get_setting_of_type(
                _self._COb, i, str(object), int(state) - 1, cSetting_boolean
            )
    ...
    def get_setting_int(name, object="", state=0, _self=cmd):  # INTERNAL
        ...
        with _self.lockcm:
            return _cmd.get_setting_of_type(
                _self._COb, i, str(object), int(state) - 1, cSetting_int
            )
    ...
    def get_setting_float(name, object="", state=0, _self=cmd):  # INTERNAL
        ...
        with _self.lockcm:
            return _cmd.get_setting_of_type(
                _self._COb, i, str(object), int(state) - 1, cSetting_float
            )
    ...
    def get_setting_text(name, object="", state=0, _self=cmd):  # INTERNAL
        ...
        with _self.lockcm:
            return _cmd.get_setting_of_type(
                _self._COb, i, str(object), int(state) - 1, cSetting_string
            )
    ...
    def get_bond(
        name, selection1, selection2=None, state=0, updates=1, quiet=1, _self=cmd
    ):
        """
        DESCRIPTION
            ...
            for model, vlist in r:
                ...
                for idx1, idx2, value in vlist:
                    if value is None:
                        continue
                    print(
                        " %s = %s between (%s`%d)-(%s`%d%s)"
                        % (name, value, model, idx1, model, idx2, suffix)
                    )
        return r
    ...
    def unset_deep(settings="", object="*", updates=1, quiet=1, _self=cmd):
        ...
        if object in ["all", "*"]:
            ...
            selection = None
            try:
                if _self.get_type(object) in ("object:group", "object:molecule"):
                    selection = "(" + object + ")"
            except:
                pass
