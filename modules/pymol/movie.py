...
def sweep(pause=0, cycles=1, _self=cmd):
    ...
    if pause > 0:
        pass_string = "1 x%d 1 -%d %d x%d %d -1" % (
            pause,
            n_state,
            n_state,
            pause,
...
def nutate(first, last, angle=30, phase=0, loop=1, shift=math.pi / 2.0, _self=cmd):
    ...
    while a < nstep:
        ...
        _self.mdo(
            "%d" % (first + a),
            "turn x,%8.3f;turn y,%8.3f;turn y,%8.3f;turn x,%8.3f"
...
def add_state_sweep(factor=1, pause=2.0, first=-1, last=-1, loop=1, start=0, _self=cmd):
    ...
    if n_frame > 0:
        ...
        cmd.mview(
            "store",
            start + n_state * factor + (n_frame * pause) / duration - 1,
            state=n_state,
            freeze=1,
        )
        cmd.mview(
            "store",
            start + n_state * factor + (2 * n_frame * pause) / duration,
            state=n_state,
            freeze=1,
        )
...
def add_state_loop(factor=1, pause=2.0, first=-1, last=-1, loop=1, start=0, _self=cmd):
    ...
    if n_frame > 0:
        ...
        cmd.mview("store", start, state=1, freeze=1)
        cmd.mview(
            "store", start + (n_frame * pause * 0.5) / duration, state=1, freeze=1
        )
        cmd.mview(
            "store",
            start + n_state * factor + (n_frame * pause * 0.5) / duration,
            state=n_state,
            freeze=1,
        )
...
def _encode(
    ...
    if done and ok and (encoder == "mpeg_encode"):
        ...
        if not ok:
            ...
            input = mpeg_encode.input(fn_rel, ".", prefix, first, last, mpeg_quality)
            ...
            if fps_legal != round(fps, 3):
                colorprinting.warning(
                    " Warning: Adjusting frame rate to {} fps (legal values are: {})".format(
                        fps_legal, FPS_LEGAL_VALUES
                    )
                )
            input = input.replace(
                "FRAME_RATE 30", "FRAME_RATE {:.3f}".format(fps_legal)
            )
        ...
        try:
            ...
            if process.returncode != 0:
                colorprinting.error(
                    "ffmpeg failed with " "exit status {}".format(process.returncode)
                )
        finally:
            os.chdir(old_cwd)
