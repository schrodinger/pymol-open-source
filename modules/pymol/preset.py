...
def simple(selection="(all)", _self=cmd):
    ...
    util.cnc(
        "(( rep lines or rep sticks or (" + lig_and_solv_sele + ")) and (" + s + "))",
        _self=cmd,
    )
...
def ligands(selection="(all)", _self=cmd):
    ...
    try:
        ...
        cmd.show(
            "lines", "(" + s + " and byres (" + host + " within 5 of " + lig + "))"
        )
        cmd.show("sticks", lig)
...
def ligand_sites(selection="(all)", _self=cmd):
    ...
    try:
        ...
        cmd.show(
            "surface", "(" + s + " and ((rep lines expand 4) within 6 of " + lig + "))"
        )
...
def interface(selection="*", _self=cmd):
    ...
    _self.select(
        ...
        % (
            s,
            " ".join(
                '((chain "%s") around 4.5) ' % (chain) for chain in _self.get_chains(s)
            ),
        ),
        0,
    )
