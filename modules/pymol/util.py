    ...
    for name in list:
        if name != target:
            if cmd.count_atoms("(%s) and (%s)" % (target, name)) == 0:
                cmd.align(
                    "polymer and name CA and (%s)" % name,
                    "polymer and name CA and (%s)" % target,
                    max_gap=max_gap,
                    quiet=0,
                    object=aln_object,
                )
...
def protein_assign_charges_and_radii(obj_name, _self=cmd):
    ...
    if not assign.amber99(obj_name, quiet=1, _self=_self):
        ...
        cmd.remove(
            obj_name + " and flag 23"
        )  # get rid of any atoms that weren't assigned
...
def protein_vacuum_esp(selection, mode=2, border=10.0, quiet=1, _self=cmd):
    ...
    if selection.split() != [selection] or selection not in cmd.get_names("objects"):
    ...
    cmd.create(
        ...
        + selection
        + "))) and resn NME+NHE+ACE)) and (not hydro)",
    )
...
def interchain_distances(
    name, selection, cutoff=None, mode=None, label=0, reset=1, _self=cmd
):
    """
    DESCRIPTION
...
def get_sasa_relative(
    ...
    *,
    ...
    try:
        ...
        for key in resarea:
            _self.create(
                tripepname, "byres (/%s/%s/%s/`%s extend 1)" % key, state, 1, zoom=0
            )
            _self.get_area(tripepname, 1, load_b=1)

            resarea_exposed = [0.0]
            _self.iterate(
                f"/{tripepname}/{key[1]}/{key[2]}/`{key[3]} & ({subsele})",
                "resarea_exposed[0] += b",
                space=locals(),
            )
        ...
        _self.alter(sele, var + " = resarea[model,segi,chain,resi]", space=locals())
        ...
        if vis:
            ...
            _self.spectrum(var, "white blue", sele, minimum=0.0, maximum=1.0)
...
def ligand_zoom(step=1, _self=cmd):
    ...
    s = {"ligand_set": set()}
    if _self.iterate("organic", "ligand_set.add((model,segi,chain,resi))", space=s) < 1:
        return
