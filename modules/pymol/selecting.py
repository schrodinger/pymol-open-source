...
if True:
    ...
    def select(
        name, selection="", enable=-1, quiet=1, merge=0, state=0, domain="", _self=cmd
    ):
        """
        DESCRIPTION
    ...
    def select_list(name, object, id_list, state=0, mode="id", quiet=1, _self=cmd):
        ...
        with _self.lockcm:
            return _cmd.select_list(
                _self._COb, name, object, id_list, int(state) - 1, int(mode), int(quiet)
            )
    ...
    def indicate(selection="(all)", _self=cmd):
        ...
        try:
            _self.lock(_self)
            r = _cmd.select(
                _self._COb, "indicate", "(" + str(selection) + ")", 1, -1, ""
            )
            if is_error(r):
