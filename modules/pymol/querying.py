...
if True:
    ...
    def get_volume_field(objName, state=1, copy=1, *, _self=cmd):
        ...
        """
        with _self.lockcm:
            r = _self._cmd.get_volume_field(
                _self._COb, objName, int(state) - 1, int(copy)
            )
        return r
    ...
    def get_volume_histogram(objName, bins=64, range=None, *, _self=cmd):
        ...
        """
        with _self.lockcm:
            r = _self._cmd.get_volume_histogram(
                _self._COb, objName, int(bins), range or (0.0, 0.0)
            )
        return r
    ...
    def get_object_matrix(object, state=1, incl_ttt=1, *, _self=cmd):
        ...
        object = str(object)
        with _self.lockcm:
            r = _cmd.get_object_matrix(
                _self._COb, str(object), int(state) - 1, int(incl_ttt)
            )
        return r
    ...
    def get_symmetry(selection="(all)", state=CURRENT_STATE, quiet=1, *, _self=cmd):
        ...
        if not quiet:
            if r:
                print(
                    " get_symmetry: A     = %7.3f B    = %7.3f C     = %7.3f"
                    % tuple(r[0:3])
                )
                print(
                    " get_symmetry: Alpha = %7.3f Beta = %7.3f Gamma = %7.3f"
                    % tuple(r[3:6])
                )
    ...
    def distance(
        ...
        *,
        ...
        if name is not None:
            if len(name):
                if (
                    name[0] == "(" or " " in name or "/" in name
                ):  # we're one argument off...
    ...
    def get_idtf(quiet=1, *, _self=cmd):
        ...
        if not quiet:
            ...
            print(
                " 3Daac=%3.1f, 3Droll=0, 3Dc2c=0 0 1, 3Droo=%1.2f, 3Dcoo=0 0 %1.2f"
                % (fov, -dist, dist)
            )
        ...
        if True:
            ...
            if quiet < 1 and _feedback(fb_module.cmd, fb_mask.results, _self):
                ...
                if quiet < 0:
                    if r[3]:
                        print(
                            " build date:", time.strftime("%c %Z", time.localtime(r[3]))
                        )
                    if r[4]:
                        print(" git sha:", r[4])
        return r
    ...
    def count_frames(quiet=1, *, _self=cmd):
        ...
        with _self.lockcm:
            ...
            if not quiet:
                print(" cmd.count_frames: %d frames" % r)
        return r

    def overlap(
        selection1, selection2, state1=1, state2=1, adjust=0.0, quiet=1, *, _self=cmd
    ):
        """
        DESCRIPTION
        ...
        with _self.lockcm:
            ...
            if not quiet:
                print(" cmd.overlap: %5.3f Angstroms." % r)
        return r
    ...
    def get_color_tuple(name, mode=0, *, _self=cmd):
        ...
        if mode in (1, 2):
            print(
                " Warning: use get_color_indices instead of get_color_tuple(mode={})".format(
                    mode
                )
            )
        elif mode == 3:
    ...
    def get_atom_coords(selection, state=ALL_STATES, quiet=1, *, _self=cmd):
        ...
        selection = selector.process(selection)
        with _self.lockcm:
            r = _cmd.get_atom_coords(
                _self._COb, str(selection), int(state) - 1, int(quiet)
            )
        if not quiet:
            for a in r:
    ...
    def get_distance(
        atom1="pk1", atom2="pk2", state=CURRENT_STATE, quiet=1, *, _self=cmd
    ):
        """
        DESCRIPTION
    ...
    def get_angle(
        ...
        *,
        ...
        with _self.lockcm:
            r = _cmd.get_angle(
                _self._COb, str(atom1), str(atom2), str(atom3), int(state) - 1
            )
        if not quiet:
    ...
    def get_area(selection="(all)", state=1, load_b=0, quiet=1, *, _self=cmd):
        ...
        with _self.lockcm:
            r = _cmd.get_area(
                _self._COb, "(" + str(selection) + ")", int(state) - 1, int(load_b)
            )
        if not quiet:
    ...
    def identify(selection="(all)", mode=0, quiet=1, *, _self=cmd):
        ...
        with _self.lockcm:
            r = _cmd.identify(
                _self._COb, "(" + str(selection) + ")", int(mode)
            )  # 0 = default mode
        if is_list(r):
            if len(r):
                if not quiet:
    ...
    def count_atoms(
        selection="(all)", quiet=1, state=ALL_STATES, domain="", *, _self=cmd
    ):
        """
        DESCRIPTION
    ...
    def get_selection_state(selection, *, _self=cmd):
        ...
        """
        state_set = set(
            map(_self.get_object_state, _self.get_object_list("(" + selection + ")"))
        )
        if len(state_set) != 1:
            if len(state_set) == 0:
                return 1
