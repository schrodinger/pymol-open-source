...
class Nucmutagenesis(Wizard):
    ...
    def __init__(self, _self=pymol.cmd):  # PyMOL pattern
        ...
        for mode in self._mode_labels:
            tmp_menu.append([1, mode, 'cmd.get_wizard().set_mode("' + mode + '")'])
        ...
        for rep in self._reps:
            tmp_menu2.append(
                [1, self._rep_name[rep], 'cmd.get_wizard().set_rep("' + rep + '")']
            )
    ...
    def get_prompt(self):
        ...
        if self._status == Status.NO_SELECTION:
            ...
            self.prompt = [
                "Select a nucleotide for %s or pick a new position..." % self.res_text
            ]
        return self.prompt
    ...
    def _get_chi_dihedral_atoms(self, base_type_lower):
        ...
        if base_type_lower in self._base_types:
            ...
            if all(
                self.cmd.count_atoms("(%s) & name %s" % (SRC_SELE, name)) == 1
    ...
    def _do_mutation(self):
        ...
        if any(
            ...
            self.clear()
            raise pymol.wizarding.WizardError("Improper selection of nucleic acid.")
        ...
        cmd.iterate(
            "(%s & name C1')" % SRC_SELE, "stored.resn = resn", space=self._space
        )
        src_type_lower = self._space["stored"].resn.lower()
        (
            self._src_O_atom_name,
            self._src_Cp_atom_name,
            self._src_N_atom_name,
            self._src_C_atom_name,
        ) = self._get_chi_dihedral_atoms(src_type_lower)

        (
            self._frag_O_atom_name,
            self._frag_Cp_atom_name,
            self._frag_N_atom_name,
            self._frag_C_atom_name,
        ) = self._get_chi_dihedral_atoms(frag_type_lower)
    ...
    def apply(self):
        ...
        for a in self._sugar_phos_atoms:
            cmd.select(
                "_tmp_sele_invert", "_tmp_sele_invert | (%s & name %s)" % (SRC_SELE, a)
            )

        cmd.select("_tmp_sele_invert", "%s and not _tmp_sele_invert" % SRC_SELE)
        ...
        cmd.fuse(
            ...
            % (
                new_name,
                self._stored.identifiers[0],
                self._stored.identifiers[1],
                self._stored.identifiers[2],
                self._src_Cp_atom_name,
            ),
            1,
        )
