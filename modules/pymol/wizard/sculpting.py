    ...
    def update_selections(self):
        if self.status == HAVE_SELECTIONS:
            if self.mode == "by_resi":
                ...
                self.cmd.select(
                    excl_sele,
                    "((byobj %s) and (not (%s|%s)))" % (cent_sele, free_sele, fix_sele),
                )
            elif self.mode == "ligand_rx":
                self.cmd.select(
                    free_sele, "((byobj %s) and byres (%s))" % (cent_sele, cent_sele)
                )
                self.cmd.select(
                    fix_sele,
                    "((byobj %s) and (not %s) and byres ((%s) x; %8.3f))"
    ...
    def get_panel(self):
        return [
            ...
            [
                2,
                "Toggle Sculpting",
                'cmd.set("sculpting",{"off":"on"}.get(cmd.get("sculpting"),0))',
            ],
            [
                2,
                "Toggle Bumps",
                'cmd.set("sculpt_vdw_vis_mode",{0:1}.get(int(cmd.get("sculpt_vdw_vis_mode")),0))',
            ],
