...
class Filter(Wizard):
    ...
    def migrate_session(self, version):
        ...
        for object, sdo in self.dict.items():
            ...
            title2ident = dict(
                (self.cmd.get_title(object, state), self.get_ident(object, state))
    ...
    def update_object_menu(self):
        ...
        for a in self.avail_objects:
            self.menu["object"].append(
                [1, a, 'cmd.get_wizard().set_object("%s")' % (a)]
            )
    ...
    def get_ident(self, object, state):
        return "%d/%d %s" % (state, self.tota, self.cmd.get_title(self.object, state))
        ...
        if self.object is None:
            ...
            try:
                ...
                print(
                    " Filter-Warning: '"
                    + fname
                    + "' in current directory is not writable."
                )
            ...
            try:
                ...
                f.write(
                    "Total\t%d\nAccepted\t%d\nRejected\t%d\nDeferred\t%d\nRemaining\t%d\n\n"
                    % (self.tota, self.acce, self.reje, self.defe, self.togo)
                )
