...
class Mutagenesis(Wizard):
    ...
    def __init__(self, _self=cmd):
        ...
        self.ind_library = io.pkl.fromFile(
        ...
        for a in self.modes:
            ...
            #                smm[3][2].append([ 1, self.mode_label[a[3:]], 'cmd.get_wizard().set_mode("'+a+'")'])
                ...
                smm.append(
                    [1, self.mode_label[a], 'cmd.get_wizard().set_mode("' + a + '")']
                )
                ...
                for b in lst:
                    if start == 0:
                        if b[1][0:] == a:
                            start = ix
                            stop = ix + 1
                    elif b[1][0:3] == a[0:3] or (
                        b[1][0:2] == a[0:2] and a[0:2] == "HI"
                    ):
                        stop = ix + 1
                    ix = ix + 1
        ...
        for a in self.n_caps:
            smm.append(
                [1, self.n_cap_name[a], 'cmd.get_wizard().set_n_cap("' + a + '")']
            )
        self.menu["n_cap"] = smm
        ...
        for a in self.c_caps:
            smm.append(
                [1, self.c_cap_name[a], 'cmd.get_wizard().set_c_cap("' + a + '")']
            )
        self.menu["c_cap"] = smm
    ...
    def load_library(self):
        if self.dep == "dep":
            if not hasattr(self, "dep_library"):
                self.dep_library = io.pkl.fromFile(
    ...
    def get_event_mask(self):
        return (
            Wizard.event_mask_pick + Wizard.event_mask_select + Wizard.event_mask_state
        )
        ...
        if self.status == 1:
            ...
            if True:
                ...
                if self.lib_mode != "current":
                    ...
                    cmd.create(
                        new_name, "?%s | ?%s" % (new_name, tmp_obj2), state, state
                    )

                    # now connect them
                    cmd.select(
                        tmp_sele2,
                        "/%s/%s/%s/%s" % ((new_name,) + self.stored.identifiers[:3]),
                    )
                    cmd.bond(
                        "?%s & name C" % (tmp_sele1),
                        "?%s & name N" % (tmp_sele2),
                    ...
                    cmd.set_geometry(
                        "(?%s | ?%s) & name C+N" % (tmp_sele1, tmp_sele2), 3, 3
                    )  # make amide planer
    ...
    def get_prompt(self):
        ...
        if self.status == 0:
            ...
            self.prompt = [
                "Select a rotamer for %s or pick a new residue..." % self.res_text
            ]
        return self.prompt
    ...
    def do_library(self):
        ...
        if self.lib_mode != "current":
            ...
            if cmd.count_atoms("(%s & name OXT)" % src_sele):
                cmd.iterate_state(
                    1, "(%s & name OXT)" % src_sele, "stored.list=[x,y,z]"
                )
                cmd.alter_state(1, "(%s & name OXT)" % frag_name, "(x,y,z)=stored.list")
            elif cmd.count_atoms(
                "(%s & name OXT)" % frag_name
            ):  # place OXT if no template exists
            ...
            if cmd.count_atoms("(hydro and bound_to (name N & (%s)))" % frag_name) == 1:
                if (
                    cmd.count_atoms("(hydro and bound_to (name N & (%s)))" % src_sele)
                    == 1
                ):
                    cmd.iterate_state(
                        1,
                        "(hydro and bound_to (name N & (%s)))" % src_sele,
                        "stored.list=[x,y,z]",
                    )
                    cmd.alter_state(
                        1,
                        "(hydro and bound_to (name N & (%s)))" % frag_name,
                        "(x,y,z)=stored.list",
                    )
                elif (
                    cmd.select(
                        tmp_sele1, "(name C & bound_to (%s and elem N))" % src_sele
                    )
                    == 1
                ):
                    ...
                    if cmd.count_atoms("name C & (%s)" % (frag_name)) == 1:
                        if self.c_cap == "amin":
                            editor.attach_amino_acid(
                                "name C & (%s)" % (frag_name), "nhh", _self=cmd
                            )
                        elif self.c_cap == "nmet":
                            editor.attach_amino_acid(
                                "name C & (%s)" % (frag_name), "nme", _self=cmd
                            )
                        if cmd.count_atoms(
            ...
            if self.n_cap in ["acet"]:
                if not cmd.count_atoms(
                    ...
                    if cmd.count_atoms("name N & (%s)" % (frag_name)) == 1:
                        if self.n_cap == "acet":
                            editor.attach_amino_acid(
                                "name N & (%s)" % (frag_name), "ace", _self=cmd
                            )
                        if cmd.count_atoms(
                            "hydro & bound_to (name N & bound_to (name C & (%s)))"
                            % frag_name
        ...
        if lib is not None:
            ...
            if self.bump_check:
                ...
                if (
                    cmd.select(
                        tmp_sele1,
                        "(name N & (%s in (neighbor %s)))" % (bump_name, src_sele),
                    )
                    == 1
                ) and (
                    cmd.select(
                        tmp_sele2, "(name C & (%s in %s))" % (bump_name, mut_sele)
                    )
                    == 1
                ):
                ...
                if (
                    cmd.select(
                        tmp_sele1,
                        "(name C & (%s in (neighbor %s)))" % (bump_name, src_sele),
                    )
                    == 1
                ) and (
                    cmd.select(
                        tmp_sele2, "(name N & (%s in %s))" % (bump_name, mut_sele)
                    )
                    == 1
                ):
                    cmd.bond(tmp_sele1, tmp_sele2)
    ...
    def do_state(self, state):
        ...
        if self.bump_scores:
            print(
                " Rotamer %d/%d, strain=%.2f"
                % (state, cmd.count_states(obj_name), self.bump_scores[state - 1])
            )
