...
class Annotation(Wizard):
    ...
    def get_event_mask(self):
        return (
            Wizard.event_mask_scene + Wizard.event_mask_state + Wizard.event_mask_frame
        )
    ...
    def get_panel(self):
        return [[1, "Annotation", ""], [2, "Dismiss", "cmd.set_wizard()"]]
...
def load_annotated_sdf(filename, object=None, state=1, discrete=1, _self=cmd):
    ...
    while 1:
        ...
        cmd.read_molstr("".join(mol_list), object, state, finish=0, discrete=discrete)
        ...
        for key in sdf_rec.kees:
            if key != "MOL":
                ...
                anno_list.append(
                    "  \\595%s: \\559%s"
                    % (key, " ".join(map(str.strip, sdf_rec.data[key])))
                )
