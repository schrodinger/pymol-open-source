    ...
    def launch(self, name, pretty_name=None):
        ...
        if hasattr(demo, name):
            ...
            if pretty_name is not None:
                cmd.do(
                    "_ wizard message, Please wait while the %s example loads..., dismiss=0"
                    % pretty_name
                )
    ...
    def get_panel(self):
        return [
            ...
            [
                ...
                'cmd.get_wizard().launch("electro","Computational Chemistry")',
            ],
            [1, "Presentation Graphics", ""],
            [
                2,
                "Molecular Animation",
                'cmd.get_wizard().launch("animate","Molecular Animation")',
            ],
            ...
            [
                ...
                'cmd.get_wizard().launch("structure","Structure Alignment")',
            ],
            [
                2,
                "Homology Modeling",
...
class DemoInfo:
    ...
    message_dict = {
        ...
        "roving_density": ["Middle-click-and-drag to move..."],
        "elec": [
        ...
        try:
            if not cleanup:
                ...
                for b in range(1, 20):
                    ...
                    for a in range(1, 5):
                        cmd.translate(
                            [xt * scale, yt * scale, 0], object="rep%d" % a, camera=0
                        )
                        xt = xt + 2
                    yt = -yt
                    xt = -3.2
                ...
                st = " ".join(
                    map(
                        lambda x, y: "rotate angle=-3,object=rep%d,axis=%s;" % (x, y),
                        list(range(1, 9)),
                        ["x", "y", "x", "y", "x", "y", "x", "y"],
                    )
                )
