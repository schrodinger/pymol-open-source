...
class Measurement(Wizard):
    ...
    neighbor_modes = ["same", "other", "enabled", "all", "in_object", "in_selection"]
    object_modes = [
        "merge",
        "overwr",
        "append",
    ...
    def get_event_mask(self):
        ...
        """
        return (
            Wizard.event_mask_pick + Wizard.event_mask_select + Wizard.event_mask_dirty
        )
    ...
    def neighbor_objects(self, a):
        ...
        for b in list:
            result.append(
                [1, b, 'cmd.get_wizard().set_neighbor_target("%s","%s")' % (a, b)]
            )
        return result
    ...
    def neighbor_submenu(self, a, _self=cmd):
        return [
            ...
            [
                1,
                "in all objects",
                'cmd.get_wizard().set_neighbor_target("' + a + '","all")',
            ],
            [
                1,
                "in object",
                self.neighbor_objects(a),
            ],  # while this is a submenu this is also a command, so [1, ...]
            [1, "in selection", self.neighbor_selections(a)],  # not [2, ...]
            [
                1,
                "in other objects",
                'cmd.get_wizard().set_neighbor_target("' + a + '","other")',
            ],
            [
                1,
                "in same object",
                'cmd.get_wizard().set_neighbor_target("' + a + '", "same")',
            ],
        ]
    ...
    def cleanup(self):
        ...
        self.clear_input()
        self.cmd.set(
            "mouse_selection_mode", self.selection_mode
    ...
    def do_pick(self, bondFlag):
        ...
        if bondFlag:
            ...
            if self.mode == "pairs":
                ...
                if self.status < 2:
                    ...
                    if (
                        ...
                        and self.cmd.get_selection_state("?pk1") != 0
                    ):
            ...
            if self.mode == "hbond":
                ...
                if cnt:
                    self.cmd.dist(
                        obj_name, "(pk1)", sele_prefix, cutoff=cutoffType, reset=reset
                    )
                else:
