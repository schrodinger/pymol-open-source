...
class Density(Wizard):
    ...
    def __init__(self, _self=cmd):
        ...
        level_menu = lambda x: [
            [1, "1.0 sigma", "cmd.get_wizard().set_level(%d,1.0)" % x],
    ...
    def update_map_menus(self):
        ...
        for a in self.map:
            ...
            for a in self.avail_maps:
                self.menu[map_kee].append(
                    [1, a, 'cmd.get_wizard().set_map(%d,"%s")' % (c, a)]
                )
            self.menu[map_kee].append(
                [1, "(none)", 'cmd.get_wizard().set_map(%d,"")' % c]
            )
            c = c + 1
        ...
        if 1:
            ...
            for a in self.map:
                ...
                if len(a) and (a in self.cmd.get_names("objects")):
                    if self.cmd.get_type(a) == "object:map":
                        self.cmd.isomesh(
                            oname, a, self.level[c], sele_name, self.radius, state=1
                        )
                    if color:
                        if c == 0:
    ...
    def next_res(self, d=1):
        ...
        if not ("_dw" in self.cmd.get_names("selections")):
            ...
            n = self.cmd.select(
                ...
                + a0.chain
                + "/"
                + str(a0.resi_number + d)
                + "/"
                + atn,
            )
            if n == 0:  # deal with gaps in sequence:
                self.cmd.select(
                    "_chain", "" + obj + "/" + a0.segi + "/" + a0.chain + "//" + atn
                )
