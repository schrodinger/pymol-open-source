    ...
    def get_prompt(self):
        ...
        if self.mode == "cpychg":
            if self.status == 0:
                ...
                self.prompt = [
                    "Pick destination atom on which to assign charge %6.4f"
                    % self.partial_charge
                ]
        if self.mode == "addchg":
            if self.status == 0:
                ...
                self.prompt = [
                    "Pick destination atom on which to add charge %6.4f"
                    % self.partial_charge
                ]
        if self.mode == "mzochg":
            if self.status == 0:
        ...
        if self.mode == "sumchg":
            ...
            if self.status == 1:
                self.prompt = [
                    "Total charge on the chain is %6.4f" % self.partial_charge,
        ...
        if "wcharge" in self.cmd.get_names("selections"):
            ...
            if self.cmd.iterate(
                "(byres wcharge)", "stored.charge = stored.charge + partial_charge"
            ):
                self.prompt.insert(
                    0, "Total charge on the residue is %6.4f" % pymol.stored.charge
                )
    ...
    def do_pick(self, bondFlag):
        ...
        if self.mode == "cbachg" or self.mode == "rbachg":
            ...
            if self.status == 0:
                ...
                if self.cmd.iterate(
                    "(byres pk1)", "stored.chg_dict[name] = partial_charge"
                ):
                ...
                if self.cmd.iterate("(byres pk1)", "stored.valid_atoms.append(name)"):
                    ...
                    for a in valid_dict.keys():
                        ...
                        if self.mode == "rbachg":
                            ...
                            self.cmd.alter(
                                "((wcharge) and name %s)" % a, "partial_charge = 0"
                            )
                    self.status = 0
                    # update labels
                    ...
                    for a in valid_dict.keys():
                        self.cmd.select(
                            "wcharge", "(wcharge or ((byres pk1) and name %s))" % a
                        )
                    self.cmd.unpick()
                    self.cmd.enable("wcharge")
        ...
        if self.mode == "addchg":
            ...
            # dropping off
                ...
                pymol.stored.charge = self.partial_charge
                if self.cmd.alter(
                    "(pk1)", "partial_charge = partial_charge + stored.charge"
                ):
                    self.status = 0
                    self.cmd.label("(pk1)", "'%6.4f'%partial_charge")
                    self.cmd.select("wcharge", "(pk1)")
        ...
        if self.mode == "sumchg":
            ...
            if self.cmd.iterate(
                "(pkmol)", "stored.charge = stored.charge + partial_charge"
            ):
                self.partial_charge = pymol.stored.charge
                self.status = 1
