    ...
    if OE_DIR is not None:
        ...
        if os.path.exists(SZYBKI_EXE):
            ...
            if os.path.exists(SZYBKI_EXE):
                result = 1
        if result < 0:
            SZYBKI_EXE = os.path.join(
                OE_DIR, "arch/microsoft-win32-i586/bin/szybki-1.0b7.exe"
            )
            if os.path.exists(SZYBKI_EXE):
                result = 1
...
class Cleanup(Wizard):
    ...
    def update_menus(self):
        ...
        target = [
            [2, "Target", ""],
            [1, "(none)", ' cmd.get_wizard().set_target("(none)")'],
        ]
    ...
    def __init__(self, _self=cmd):
        ...
        if auto_configure() < 1:
            raise pymol.CmdException(
                'cannot find "szybki" executable, please '
    ...
    def run(self):
        ...
        if self.ligand in self.cmd.get_names("objects"):
            ...
            self.cmd.save(
                inp, self.ligand
            )  # MOL file will be written using internal order
            while 1:
