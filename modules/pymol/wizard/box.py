...
class Box(Wizard):
    ...
    def auto_position(self, fract=0.75, size=1.0):
        ...
        if self.points_name in self.cmd.get_names():
            ...
            if fract < 0.5:
                fract = 1.0 - fract
            if fract > 0.995:
                fract = 0.995  # minimum 1% distance from clipping planes
            ...
            plane_size = (
                size * (one_minus_fract * view[15] + fract * view[16]) * tan_half_fov
            )
            ...
            plane = list(
                map(lambda p, v=view: [v[12] + p[0], v[13] + p[1], v[14] + p[2]], plane)
            )
    ...
    def set_name(self, name):
        ...
        if self.points_name not in self.cmd.get_names():
            ...
            self.cmd.set(
                "surface_mode", 0, self.points_name
            )  # make sure no surface is shown
            self.coord = None
    ...
    def update_box(self):
        ...
        if self.points_name in self.cmd.get_names():
            ...
            self.cmd.order(
                self.cgo_name + " " + self.points_name, sort=1, location="bottom"
            )
            self.cmd.set(
                "nonbonded_size",
