...
def load_pdbml(
    filename, object="", discrete=0, multiplex=1, zoom=-1, quiet=1, _self=cmd
):
    """
    DESCRIPTION
    ...
    try:
        ...
        atom_site_list = root.findall(
            "." "/" + PDBxNS + "atom_siteCategory" "/" + PDBxNS + "atom_site"
        )
    ...
    # symmetry and cell
    try:
        node = root.findall("." "/" + PDBxNS + "cellCategory" "/" + PDBxNS + "cell")[0]
        cell = [
...
def get_stlstr(binary=1, quiet=0, _self=cmd):
    ...
    DESCRIPTION
        ...
        STL geometry export
    """
    raise pymol.IncentiveOnlyException("STL export not supported by this PyMOL build")
...
def read_stlstr(contents, object, state=0, zoom=-1, _self=cmd):
    ...
    """
    raise pymol.IncentiveOnlyException("STL import not supported by this PyMOL build")
