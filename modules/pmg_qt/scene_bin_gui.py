...
class ScenePanel(QtWidgets.QWidget):
    ...
    def _populate_data(self):
        ...
        for scene_name in self.scene_list:
            ...
            self.sceneTableWidget.setItem(
                row_number,
                SceneTableColumn.MESSAGE,
                QtWidgets.QTableWidgetItem(scene_message),
            )
            self.sceneTableWidget.setItem(
                row_number,
                SceneTableColumn.ACTIONS,
    ...
    def _update_scene_dict(self):
        ...
        remove_scenes = [
            scene_name
            for scene_name in self.scene_dict
    ...
    def _format_condensed_widget(self):
        ...
        self.sceneTableWidget.setHorizontalHeaderLabels(["Name", "Scene Preview"])
    ...
    def _update_scene(self):
        ...
        if selection:
            ...
            self.scene_dict[name][SceneDictIndex.QPIXMAP] = self._get_scene_png(name)
            self._update_table()
    ...
    def _get_table_scene_list(self):
        ...
        for row_num in range(self.sceneTableWidget.rowCount()):
            ...
            position = int(self.sceneTableWidget.rowViewportPosition(row_num) / 100)
    ...
    def _check_table_state(self):
        ...
        diff_list = self._compare_scene_lists(self.scene_list, scene_ordered_list)
        if len(diff_list) > 1:
    ...
    def _selection_changed(self):
        ...
        """
        item_selected = bool(self.sceneTableWidget.selectionModel().selectedIndexes())
    ...
    def _rename_scene(self, item):
        ...
        if " " in item.text():
            ...
            diff_list = self._compare_scene_lists(
                self.cmd.get_scene_list(), self._get_table_scene_list()
            )
            if diff_list:
