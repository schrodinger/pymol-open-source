...
class PyMOLShortcutMenu(QtWidgets.QWidget):
    ...
    def delete_selected(self):
        ...
        for ind, table_index in enumerate(list_indexes):
            ...
            if delete_key in self.shortcut_manager.default_bindings:
                ...
                self.shortcut_manager.cmd.shortcut_dict[delete_key][
                    ShortcutIndex.USER_DEF
                ] = "Deleted"
            else:
                self.model.removeRow(table_index.row())
                print(
                    delete_key, " has been deleted and will be removed from the table"
                )
    ...
    def reset_selected(self):
        ...
        for ind, table_index in enumerate(list_indexes):
            ...
            if reset_key not in self.shortcut_manager.default_bindings:
                ...
                reset_command = self.shortcut_manager.cmd.shortcut_dict[reset_key][
                    ShortcutIndex.COMMAND
                ]
                reset_description = self.shortcut_manager.cmd.shortcut_dict[reset_key][
                    ShortcutIndex.DESCRIPT
                ]

                self.table.model().setData(table_colm_command, reset_command)
                self.table.model().setData(table_colm_descipt, reset_description)

                self.shortcut_manager.cmd.shortcut_dict[reset_key][
                    ShortcutIndex.USER_DEF
                ] = ""
    ...
    def eventFilter(self, source, event):
        ...
        if (
            ...
            and source is self.create_new_form.keyEdit
        ):
    ...
    def confirm_menu_connect(self):
        self.confirm_change.confirmButton.clicked.connect(
            lambda: self.shortcut_manager.create_new_shortcut(
                self.confirm_new_key, self.confirm_new_binding
            )
        )
            ...
            if item.column() == 1 and item.text() != "Deleted":
                ...
                if not filter_active:
                    self.table.model().setData(
                        self.table.model().index(item.row(), 2), "user defined"
                    )
                else:
                    pass
