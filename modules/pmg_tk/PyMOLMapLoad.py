...
class PyMOLMapLoad:
    ...
    def pack(self):
        ...
        self._d.geometry(
            "+%d+%d" % (self._app.winfo_reqwidth(), self._app.winfo_reqheight())
        )
        self._d.withdraw()
        ...
        if not len(FCols):
            FCols = [""]
        self._ampl_chooser = Pmw.ComboBox(
            self._col_gp.interior(),
        ...
        if not len(PCols):
            PCols = [""]
        self._phase_chooser = Pmw.ComboBox(
            self._col_gp.interior(),
            label_text="Phases",
            labelpos="nw",
        ...
        if self._fileData.reso_max != None:
            ...
            default_max_res = ""
        self._min_res_fld = Pmw.EntryField(
            self._input_gp.interior(),
            labelpos="wn",
        ...
        self._max_res_fld = Pmw.EntryField(
            self._input_gp.interior(),
            labelpos="wn",
        ...
        # MAP OPTIONS GROUP
        #
        self._options_gp = Pmw.Group(
            self._d.interior(),
            tag_text="Map Options",
        )
        self._options_gp.pack(fill="x", expand="yes")

        self._name_prefix_fld = Pmw.EntryField(
            self._options_gp.interior(),
            labelpos="wn",
        ...
        self._fofc_chooser = Pmw.RadioSelect(
            self._options_gp.interior(),
...
select an amplitude column name from the file and try again.
            ...
            if r == None or r == "None" or r == "":
                print(
                    " MapLoad-Error: PyMOL could not load the MTZ file '%s' due to an unspecified error."
                    % self._fileName
                )
                print(
                    " MapLoad-Error: This typically occurs with bad data or blank column names. Please try again"
                )
                print(
                    " MapLoad-Error: or contact 'help@schrodinger.com' for more information."
                )
                return None
            ...
            try:
                ...
                if self._fofc:
                    ...
                    if toShow == "isosurface":
                        pymol.cmd.isosurface(
                            pymol.cmd.get_unused_name(r + "-srf"), pfx, level=1.0
                        )
                    elif toShow == "isomesh":
