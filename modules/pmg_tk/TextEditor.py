...
class TextEditor:
    ...
    def doSaveAs(self, *args):
        handle = tkFileDialog.asksaveasfile(
            mode="w",
    ...
    def onClose(self):
        if self._get() != self._savedcontent:
            ok = tkMessageBox.askyesnocancel(
                "Save?", "Save before quit?", parent=self.root
            )
            if ok:
                self.doSave()
            elif ok is None:
    ...
    def __init__(self, parent=None, filename="", title="Text Editor"):
        ...
        filemenu.add_command(
            label="Save as ...", command=self.doSaveAs, accelerator="Ctrl+Shift+S"
        )
...
def _edit_pymolrc(app, _list=()):
    try:
        ...
        pymolrc = tkSimpleDialog.askstring(
            "Create new pymolrc?", "Filename of new pymolrc", initialvalue=pymolrc
        )
