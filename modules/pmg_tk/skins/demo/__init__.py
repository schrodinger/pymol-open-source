...
class Demo(PMGSkin):
    ...
    def createMenuBar(self):
        self.menuBar = Pmw.MenuBar(
            self.root, hull_relief=RAISED, hull_borderwidth=1  # balloon=self.balloon,
        )
        ...
        self.menuBar.addmenuitem(
            "File", "command", "Open file", label="Open...", command=self.fileOpenDialog
        )

        self.menuBar.addcascademenu("File", "Skin", "Skin", label="Skin", tearoff=TRUE)

        self.app.addSkinMenuItems(self.menuBar, "Skin")

        self.menuBar.addmenuitem(
            "File", "command", "Quit PyMOL", label="Quit", command=self.quit
        )
