...
class Normal(PMGSkin, pymol._gui.PyMOLDesktopGUI):
    ...
    def inc_fontsize_dialog(self):
        ...
        Button(grid, text=" - ", command=lambda: self.inc_fontsize(-1)).grid(
            column=col(), **kw
        )
        Button(grid, text=" + ", command=lambda: self.inc_fontsize(1)).grid(
            column=col(), **kw
        ...
        col = getattr(iter(range(5)), _next_method_name)
        Button(
            grid, text=" - ", command=lambda: self._inc_fontsize(-1, self.fixedfont)
        ).grid(column=col(), **kw)
        Button(
            grid, text=" + ", command=lambda: self._inc_fontsize(1, self.fixedfont)
        ).grid(column=col(), **kw)
        ...
        self.cmd.cd(
            encode(
                tkFileDialog.askdirectory(
                    title="Change Working Directory", initialdir=self.initialdir
                )
            )
            or ".",
            quiet=0,
    ...
    def createDataArea(self):
        ...
        self.dataArea = self.app.createcomponent(
            "dataarea", (), None, Frame, (self.app._hull,), relief=SUNKEN, bd=1
        )
        self.dataArea.pack(side=LEFT, fill=BOTH, expand=YES, padx=1, pady=1)
    ...
    def createCommandArea(self):
        ...
        self.commandFrame = self.app.createcomponent(
            "commandframe", (), None, Frame, (self.app._hull,), relief=SUNKEN, bd=1
        )
        self.commandFrame.place(width=500)
        self.commandFrame.pack(side=TOP, expand=NO, fill=BOTH, padx=1, pady=1)

    def destroyCommandArea(self):
        self.app.destroycomponent("commandframe")

    def createMessageBar(self):
        self.messageBar = Pmw.MessageBar(
            self.commandFrame,
            entry_width=25,
            entry_relief="sunken",
            entry_borderwidth=1,
        ...
        self.abortButton = Button(
            ...
            command=lambda s=self: self.abort(),
            padx=0,
            pady=0,
        )
        ...
        self.abortButton = Button(
            ...
            highlightthickness=0,
            #                                state=DISABLED,
            command=lambda s=self: self.rebuild(),
            padx=0,
            pady=0,
        )
        self.abortButton.pack(side=RIGHT, fill=BOTH, expand=YES)
    ...
    def get_current_session_file(self):
        ...
        session_file = session_file.replace(
            "\\", "/"
    ...
    def confirm_quit(self, e=None):
        if self.cmd.get_setting_boolean("session_changed"):
            ...
            if session_file != "":
                message = (
                    "Save the current session '%s'?" % os.path.split(session_file)[1]
                )
            else:
                message = "Save the current session?"
            check = tkMessageBox._show(
                "Save Session", message, tkMessageBox.QUESTION, tkMessageBox.YESNOCANCEL
            )
    ...
    def buttonAdd(self, frame, text, cmmd):
        newBtn = Button(
            frame, text=text, highlightthickness=0, command=cmmd, padx=0, pady=0
        )
    ...
    def createButtons(self):
        ...
        row1 = self.app.createcomponent(
            "row1", (), None, Frame, self.commandFrame, bd=0
        )
        row1.pack(side=TOP, fill=BOTH, expand=YES)
        btn_reset = self.buttonAdd(row1, "Reset", lambda s=self: s.cmd.do("_ reset"))
        btn_reset = self.buttonAdd(
            row1, "Zoom", lambda s=self: s.cmd.do("_ zoom animate=-1")
        )
        btn_orient = self.buttonAdd(
            row1, "Orient", lambda s=self: s.cmd.do("_ orient animate=1")
        )
        btn_rtrace = self.buttonAdd(row1, "Draw", lambda s=self: s.cmd.do("_ draw"))
        btn_rtrace = self.buttonAdd(
            row1, "Ray", lambda s=self: s.cmd.do("_ ray async=1")
        )

        row2 = self.app.createcomponent(
            "row2", (), None, Frame, self.commandFrame, bd=0
        )
        row2.pack(side=TOP, fill=BOTH, expand=YES)
        btn_unpick = self.buttonAdd(row2, "Unpick", lambda s=self: s.cmd.do("_ unpick"))
        btn_hidesele = self.buttonAdd(
            row2, "Deselect", lambda: self.cmd.do("_ deselect")
        )
        btn_reset = self.buttonAdd(row2, "Rock", lambda s=self: s.cmd.do("_ rock"))
        btn_getview = self.buttonAdd(
            row2, "Get View", lambda s=self: s.get_view()
        )  # doesn't get logged

        row3 = self.app.createcomponent(
            "row3", (), None, Frame, self.commandFrame, bd=0
        )
        ...
        row4 = self.app.createcomponent(
            "row4", (), None, Frame, self.commandFrame, bd=0
        )
        row4.pack(side=TOP, fill=BOTH, expand=YES)
        self.cmdB = self.buttonAdd(
            row4, "Command", lambda s=self: s.toggleFrame(s.cmdFrame)
        )
        self.buildB = self.buttonAdd(
            row4, "Builder", lambda s=self: s.toggleFrame(s.buildFrame)
        )
        self.volB = self.buttonAdd(row4, "Volume", self.newVolumeFrame)
        # initialize disabled
    ...
    def createConsole(self):
        ...
        self.entry_label = Label(
            self.entryFrame, text="PyMOL>", padx=1, pady=1, justify=RIGHT
        )
        self.entry_label.pack(side=LEFT, expand=NO, fill=X)
        self.entry = Entry(
            self.entryFrame, justify=LEFT, width=70, textvariable=self.command
        )
        ...
        self.entry.bind(
            "<Return>",
            lambda e, s=self: (s.doTypedCommand(s.command.get()), s.command.set("")),
        )
...
""",
        ...
        self.app.bind_all(
            "<Control-F1>", lambda e, s=self: s.cmd.do("cmd._special(1,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F2>", lambda e, s=self: s.cmd.do("cmd._special(2,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F3>", lambda e, s=self: s.cmd.do("cmd._special(3,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F4>", lambda e, s=self: s.cmd.do("cmd._special(4,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F5>", lambda e, s=self: s.cmd.do("cmd._special(5,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F6>", lambda e, s=self: s.cmd.do("cmd._special(6,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F7>", lambda e, s=self: s.cmd.do("cmd._special(7,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F8>", lambda e, s=self: s.cmd.do("cmd._special(8,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F9>", lambda e, s=self: s.cmd.do("cmd._special(9,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F10>", lambda e, s=self: s.cmd.do("cmd._special(10,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F11>", lambda e, s=self: s.cmd.do("cmd._special(11,0,0,2)")
        )
        self.app.bind_all(
            "<Control-F12>", lambda e, s=self: s.cmd.do("cmd._special(12,0,0,2)")
        )
        ...
        self.entry.bind(
            "<Control-Prior>", lambda e, s=self: s.cmd.do("cmd._special(104,0,0,2)")
        )
        self.entry.bind(
            "<Control-Next>", lambda e, s=self: s.cmd.do("cmd._special(105,0,0,2)")
        )
    ...
    def file_open(self, tutorial=0):
        ...
        if TkVersion > 8.3:
            ofile_list = askopenfilename(
                initialdir=initdir, filetypes=ftypes, multiple=1
            )  # new option in Tk 8.4
        else:
            ofile_list = [askopenfilename(initialdir=initdir, filetypes=ftypes)]
    ...
    def log_open(self):
        sfile = asksaveasfilename(
            initialfile=self.log_file,
    ...
    def session_save(self):
        ...
        if self.save_file != "":
            ...
            self.cmd.do(
                "_ cmd.save('''%s''','','pse',quiet=0)" % self.save_file
    ...
    def session_save_as(self):
        (self.initialdir, self.save_file) = os.path.split(
            self.get_current_session_file()
        )
        (save_file, def_ext) = os.path.splitext(self.save_file)
        sfile = asksaveasfilename(
            defaultextension=_def_ext(def_ext),
    ...
    def file_save(self):
        ...
        multiple_files_option = Pmw.RadioSelect(
            dialog.interior(),
            labelpos="w",
            orient="vertical",
            selectmode="single",
        ...
        if True:
            ...
            if multiple_files_flag == "one file" and len(sels) >= 1:
                sfile = (
                    "_".join(sels)
                    if len(sels) < 3
                    else sels[0] + "-and-%d-more" % (len(sels) - 1)
                )
                sfile = asksaveasfilename(
                    defaultextension=_def_ext(".pdb"),
                    initialfile=sfile,
                    initialdir=self.initialdir,
                ...
                if len(sfile):
                    ...
                    if state_flag == "all":
                        ...
                        for sel in sels:
                            ...
                            self.cmd.multisave(
                                sfile, str(sel), state=s, quiet=0, append=ap
                            )
                            ap = 1
                    else:
                        ...
                        if len(sfile):
                            ...
                            if doSplit:
                                ...
                                for stateSave in range(1, int(s) + 1):
                                    save_file = sfile
                                    # _state004
                                    inter = "_state" + str(stateSave).zfill(
                                        len(str(s)) + 1
                                    )
    ...
    def file_save_mpeg(self):
        ...
        def command(value):
            ...
            if len(sfile):
                ...
                self.cmd.log(
                    "movie.produce %s,quality=%d,quiet=0\n" % (sfile, mQual),
                    "cmd.movie.produce('''%s''',quality=%d,quiet=0)\n" % (sfile, mQual),
                )
                self.cmd.movie.produce(
                    sfile,
                    mode,
                    quality=mQual,
                    quiet=0,
        ...
        Label(
            parent,
            text="Encoding Quality (0-100)",
        ).grid(column=0, **gridkw)
        w_quality = Pmw.Counter(
            parent,
    ...
    def file_save_mpng(self):
        sfile = asksaveasfilename(
            initialdir=self.initialdir, filetypes=[("Numbered PNG Files", "*.png")]
        )
        if len(sfile):
            self.initialdir = os.path.dirname(sfile)
            self.cmd.log("mpng %s\n" % sfile, "cmd.mpng('%s')\n" % sfile)
    ...
    def toggleClickThrough(self, toggle):
        if toggle:
            os.system("defaults write com.apple.x11 wm_click_through -bool true")
            os.system("defaults write org.x.X11 wm_click_through -bool true")
            os.system("defaults write com.apple.x11 wm_ffm -bool true")
            os.system("defaults write org.x.X11 wm_ffm -bool true")
            print("Enabled wm_click_through and wm_ffm.", end=" ")
        else:
            os.system("defaults write com.apple.x11 wm_click_through -bool false")
            os.system("defaults write org.x.X11 wm_click_through -bool false")
            os.system("defaults write com.apple.x11 wm_ffm -bool false")
            os.system("defaults write org.x.X11 wm_ffm -bool false")
            print("Disabled wm_click_through and wm_ffm.", end=" ")
        print("Please restart X11.")

    def createMenuBar(self):
        self.menuBar = Pmw.MenuBar(
            self.root, balloon=self.balloon, hull_relief=RAISED, hull_borderwidth=1
        )
            ...
            for item in data:
                if item[0] == "separator":
                    ...
                    self.menuBar.addcascademenu(
                        parent, menulabel, label, label=label, tearoff=FALSE
                    )
                    ...
                    if command is None:
                        ...
                        addmenuitem(
                            parent, "command", label, label=label, command=command
                        )
                elif item[0] == "check":
                    label = item[1]
                    ...
                    if len(item) > 4:
                        ...
                        addmenuitem(
                            parent, "checkbutton", label, label=label, variable=var
                        )
                elif item[0] == "radio":
                    label = item[1]
                    var = getattr(self.setting, item[2])
                    value = item[3]
                    addmenuitem(
                        parent, "radiobutton", label=label, value=value, variable=var
                    )
                elif DEBUG:
                    print("error:", item)
        ...
        if (
            sys.platform == "win32"
            and self.app.pymol.invocation.options.incentive_product
        ):
        ...
        if sys.platform == "darwin":
            ...
            self.menuBar.addcascademenu(
                "Mouse", "MacX11Focus", "Mac OS X11", label="Mac OS X11"
            )
            self.menuBar.addmenuitem(
                "MacX11Focus",
                "command",
                ...
                command=lambda s=self: s.toggleClickThrough(1),
            )
            ...
            self.menuBar.addmenuitem(
                ...
                command=lambda s=self: s.toggleClickThrough(0),
            )
    ...
    def __init__(self, app):
        ...
        Normal.appversion += (
            " Incentive Product"
            if app.pymol.invocation.options.incentive_product
            else " Open-Source"
        )
        self.app = app
        self.save_file = ""
