    ...
    def run_job(self):
        if active_sele in self.cmd.get_names("selections"):
            ...
            if len(obj_list) == 1:
                ...
                self.cmd.refresh_wizard()
                self.cmd.do(
                    "_ cmd.clean('%s',message='''Cleaning %s...''',async_=1)"
                    % (active_sele, obj_list[0])
                )
...
class SculptWizard(ActionWizard):
    ...
    def sculpt_deactivate(self):
        if (self.sculpt_object != None) and self.sculpt_object in self.cmd.get_names():
    ...
    def scramble(self, mode):
        if self.cmd.count_atoms(self.sculpt_object):
            ...
            if mode == 0:
                self.cmd.select(
                    sc_tmp, self.sculpt_object + " and not (fixed or restrained)"
                )
            if mode == 1:
...
class BondWizard(RepeatableActionWizard):
    ...
    def do_pick(self, bondFlag):
        ...
        if picked == ["pk1", "pk2"]:
            if (
                (self.cmd.count_atoms("pk1 and hydro") == self.cmd.count_atoms("pk1"))
                and (self.cmd.count_atoms("(pk1 extend 1) and not hydro") == 1)
                and (
                    self.cmd.count_atoms("pk2 and hydro") == self.cmd.count_atoms("pk2")
                )
                and (self.cmd.count_atoms("(pk2 extend 1) and not hydro") == 1)
            ):
        ...
        if active_sele in self.cmd.get_names("selections"):
            if mode == 0:
                self.cmd.flag(
                    ...
                    "(( byobj "
                    + active_sele
                    + " ) and not flag %d) extend 1" % self.flag,
                    "clear",
                )
            elif mode == 1:
                self.cmd.flag(
    ...
    def do_cas(self, mode):
        if active_sele in self.cmd.get_names("selections"):
            if mode == 1:
                ...
                self.cmd.flag(
                    self.flag, active_sele + " and polymer and name ca", "set"
                )
            elif mode == 0:
                self.cmd.flag(
                    self.flag,
    ...
    def do_more(self, mode):
        if active_sele in self.cmd.get_names("selections"):
            if mode == 0:
                self.cmd.flag(
                    self.flag,
                    active_sele + " and (flag %d extend 1)" % self.flag,
                    "set",
                )
            elif mode == 1:
                self.cmd.flag(
                    ...
                    "byres ("
                    + active_sele
                    + " and (byres flag %d) extend 1)" % self.flag,
                    "set",
                )
                ...
                self.cmd.flag(
                    self.flag,
                    "byres (" + active_sele + " and flag %d )" % self.flag,
                    "set",
                )
...
class GuiButton:
    def __init__(self, frame, text, command, comment="", colspan=1):
        ...
        b = Button(
            ...
            text=text,
            padx=1,
            pady=1,  # note pady=0 better with older Xft due to scaling
            width=width,
            borderwidth=2,
            relief=RIDGE,
            command=command,
        )
        b.grid(row=frame.row, column=frame.col, columnspan=colspan, sticky=W + E)
        balloon = Pmw.Balloon()
...
class GuiImgButton:
    def __init__(self, frame, img, command, comment="", colspan=1):
        ...
        b.img = img
        b.grid(row=frame.row, column=frame.col, columnspan=colspan, sticky=S + W + E)
        balloon = Pmw.Balloon()
        balloon.bind(b, comment)
        frame.nextColumn(colspan)
...
class AtomFrame(GuiFrame):
    def __init__(self, parent):
        ...
        #        GuiLabel(self, "Atoms")
        GuiButton(
            self, "H", lambda s=self: s.replace("H", 1, 1, "Hydrogen"), "Hydrogen"
        )
        GuiButton(self, "C", lambda s=self: s.replace("C", 4, 4, "Carbon"), "Carbon")
        GuiButton(
            self, "N", lambda s=self: s.replace("N", 4, 3, "Nitrogen"), "Nitrogen"
        )
        GuiButton(self, "O", lambda s=self: s.replace("O", 4, 2, "Oxygen"), "Oxygen")
        GuiButton(
            self, "P", lambda s=self: s.replace("P", 4, 3, "Phosphorous"), "Phosphorous"
        )
        GuiButton(self, "S", lambda s=self: s.replace("S", 2, 2, "Sulfur"), "Sulfur")
        GuiButton(
            self, "F", lambda s=self: s.replace("F", 1, 1, "Fluorine"), "Fluorine"
        )
        GuiButton(
            self, "Cl", lambda s=self: s.replace("Cl", 1, 1, "Chlorine"), "Chlorine"
        )
        GuiButton(
            self, "Br", lambda s=self: s.replace("Br", 1, 1, "Bromine"), "Bromine"
        )
        GuiButton(self, "I", lambda s=self: s.replace("I", 1, 1, "Iodine"), "Iodine")
        GuiButton(
            self,
            "-CF3",
            lambda s=self: s.grow("trifluoromethane", 4, 0, "trifluoro"),
            "Trifluoromethane",
        )
        GuiButton(
            self, "-OMe", lambda s=self: s.grow("methanol", 5, 0, "methoxy"), "Methanol"
        )
...
class FragmentFrame(GuiFrame):
    def __init__(self, parent):
        ...
        GuiButton(
            self, "CH4", lambda s=self: s.grow("methane", 1, 0, "methyl"), "Methane"
        )
        GuiButton(
            self, "C=C", lambda s=self: s.grow("ethylene", 4, 0, "vinyl"), "Ethylene"
        )
        GuiButton(
            self, "C#C", lambda s=self: s.grow("acetylene", 2, 0, "alkynl"), "Acetylene"
        )
        GuiButton(
            self, "C#N", lambda s=self: s.grow("cyanide", 2, 0, "cyano"), "Cyanide"
        )
        GuiButton(
            self,
            "C=O",
            lambda s=self: s.grow(
                "formaldehyde",
                2,
                0,
                "carbonyl",
        ...
        GuiButton(
            self,
            "C=OO",
            lambda s=self: s.grow("formic", 4, 0, "carboxyl"),
            "Formic Acid",
        )
        GuiButton(
            self,
            "C=ON",
            lambda s=self: s.grow("formamide", 5, 0, "C->N amide"),
            "C->N amide",
        )
        GuiButton(
            self,
            "NC=O",
            lambda s=self: s.grow("formamide", 3, 1, "N->C amide"),
            "N->C amide",
        )
        GuiButton(
            self, "S=O2", lambda s=self: s.grow("sulfone", 3, 1, "sulfonyl"), "Sulfone"
        )
        GuiButton(
            self,
            "P=O3",
            lambda s=self: s.grow("phosphite", 4, 0, "phosphoryl"),
            "Phosphite",
        )
        ...
        GuiImgButton(
            self,
            "cyc3",
            lambda s=self: s.grow("cyclopropane", 4, 0, "cyclopropyl"),
            "Cyclopropane",
        )
        GuiImgButton(
            self,
            "cyc4",
            lambda s=self: s.grow("cyclobutane", 4, 0, "cyclobutyl"),
            "Cyclobutane",
        )
        GuiImgButton(
            self,
            "cyc5",
            lambda s=self: s.grow("cyclopentane", 5, 0, "cyclopentyl"),
            "Cyclopentane",
        )
        GuiImgButton(
            self,
            "cyc6",
            lambda s=self: s.grow("cyclohexane", 7, 0, "cyclohexyl"),
            "Cyclohexane",
        )
        GuiImgButton(
            self,
            "cyc7",
            lambda s=self: s.grow("cycloheptane", 8, 0, "cycloheptyl"),
            "Cycloheptane",
        )
        # self.nextRow()
        GuiImgButton(
            self,
            "aro5",
            lambda s=self: s.grow("cyclopentadiene", 5, 0, "cyclopentadienyl"),
            "Cyclopentadiene",
        )
        GuiImgButton(
            self, "aro6", lambda s=self: s.grow("benzene", 6, 0, "phenyl"), "Benzene"
        )
        GuiImgButton(
            self, "aro65", lambda s=self: s.grow("indane", 12, 0, "indanyl"), "Indane"
        )
        GuiImgButton(
            self,
            "aro66",
            lambda s=self: s.grow("napthylene", 13, 0, "napthyl"),
            "Napthylene",
        )
        GuiImgButton(
            self,
            "aro67",
            lambda s=self: s.grow("benzocycloheptane", 13, 0, "benzocycloheptyl"),
            "Benzocycloheptane",
        )
        GuiButton(self, "", None, "Unassigned")
        GuiButton(self, "", None, "Unassigned")
...
class ModifyFrame(GuiFrame):
    def __init__(self, parent):
        ...
        # self.nextRow()
        GuiButton(
            self, "|", lambda s=self: s.setOrder("1", "single"), "Create single bond"
        )
        GuiButton(
            self,
            "||",
            lambda s=self: s.setOrder(
                "2",
                "double",
            ),
            "Create double bond",
        ...
        GuiButton(
            self, "|||", lambda s=self: s.setOrder("3", "triple"), "Create triple bond"
        )
        #        GuiButton(self, "Cycle", lambda s=self: s.cmd.cycle_valence(1), "Cycle valence (single -> double -> triple")
        GuiButton(
            self,
            "Arom",
            lambda s=self: s.setOrder("4", "aromatic"),
            "Create aromatic bond",
        )
...
class AminoAcidFrame(GuiFrame):
    ...
    def __init__(self, parent):
        ...
        for aa in aaList:
            r = aa.lower()
            GuiButton(
                self, aa, lambda s=self, r=r: s.attach(r), "Build %s residue" % aa
            )
...
class SecStructFrame(GuiFrame):
    def __init__(self, parent):
        ...
        GuiButton(
            self, "Next...", self.toggleSS, "Toggle secondary structure for building"
        )
...
class Builder(Frame):
    ...
    def constructMain(self):
        self.chemB = Button(
            self, text="Chemical", relief=SUNKEN, command=self.toggleChemProtein
        )
        self.chemB.grid(row=1, column=1, sticky=N + S + W + E)

        self.protB = Button(
            self, text="Protein", command=lambda: self.toggleChemProtein(1)
        )
        ...
        Checkbutton(
            ...
            pady=0,
            justify=LEFT,
            variable=self.electroMode,
            onvalue=1,
            offvalue=0,
            command=self.doElectro,
