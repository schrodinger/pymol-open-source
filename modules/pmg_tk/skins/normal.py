...
class Normal(PMGSkin):
   ...
   def createButtons(self):
      ...
      btn_reset = self.buttonAdd(row2,'Reset',lambda s=self: s.cmd.do("_ reset"))
      btn_reset = self.buttonAdd(row2,'Zoom',lambda s=self: s.cmd.do("_ zoom animate=1"))      
      btn_rtrace = self.buttonAdd(row2,'Ray',lambda s=self: s.cmd.do("_ ray"))
      ...
      btn_unpick = self.buttonAdd(row3,'Unpick',lambda s=self: s.cmd.do("_ unpick"))
      ...
      btn_rewind = self.buttonAdd(row1,'|<',lambda s=self: s.cmd.do("_ rewind"))
      btn_back = self.buttonAdd(row1,'<',lambda s=self: s.cmd.do("_ backward"))
      btn_stop = self.buttonAdd(row1,'Stop',lambda s=self: s.cmd.do("_ mstop"))
      btn_play = self.buttonAdd(row1,'Play',lambda s=self: s.cmd.do("_ mplay"))
      btn_forward = self.buttonAdd(row1,'>',lambda s=self: s.cmd.do("_ forward"))
...
#         win.show()
   ...
   def createMain(self):
      ...
      self.output = self.app.createcomponent('output', (), None,
                           Pmw.ScrolledText,
                           (self.dataArea,))
      ...
      self.app.bind_all('<F1>',lambda s=self: s.cmd.do("cmd._special(1,0,0)"))
      self.app.bind_all('<F2>',lambda s=self: s.cmd.do("cmd._special(2,0,0)"))
      self.app.bind_all('<F3>',lambda s=self: s.cmd.do("cmd._special(3,0,0)"))
      self.app.bind_all('<F4>',lambda s=self: s.cmd.do("cmd._special(4,0,0)"))
      self.app.bind_all('<F5>',lambda s=self: s.cmd.do("cmd._special(5,0,0)"))
      self.app.bind_all('<F6>',lambda s=self: s.cmd.do("cmd._special(6,0,0)"))
      self.app.bind_all('<F7>',lambda s=self: s.cmd.do("cmd._special(7,0,0)"))
      self.app.bind_all('<F8>',lambda s=self: s.cmd.do("cmd._special(8,0,0)"))
      self.app.bind_all('<F9>',lambda s=self: s.cmd.do("cmd._special(9,0,0)"))
      self.app.bind_all('<F10>',lambda s=self: s.cmd.do("cmd._special(10,0,0)"))
      self.app.bind_all('<F11>',lambda s=self: s.cmd.do("cmd._special(11,0,0)"))
      self.app.bind_all('<F12>',lambda s=self: s.cmd.do("cmd._special(12,0,0)"))

      self.entry.bind('<Prior>',lambda s=self: s.cmd.do("cmd._special(104,0,0)"))
      self.entry.bind('<Next>',lambda s=self: s.cmd.do("cmd._special(105,0,0)"))
      self.entry.bind('<Home>',lambda s=self: s.cmd.do("cmd._special(106,0,0)"))
      self.entry.bind('<End>',lambda s=self: s.cmd.do("cmd._special(107,0,0)"))
   ...
   def transparency_menu(self,name,label,setting_name):
      ...
      self.menuBar.addmenuitem(name,  'command', 'Off',
                               label='Off',
                               command = lambda s=self: s.cmd.do("_ cmd.set('%s',0.0)"%setting_name))

      self.menuBar.addmenuitem(name,  'command', '20%',
                               label='20%',
                               command = lambda s=self: s.cmd.do("_ cmd.set('%s',0.2)"%setting_name))

      self.menuBar.addmenuitem(name,  'command', '40%',
                               label='40%',
                               command = lambda s=self: s.cmd.do("_ cmd.set('%s',0.4)"%setting_name))

      self.menuBar.addmenuitem(name,  'command', '50%',
                               label='50%',
                               command = lambda s=self: s.cmd.do("_ cmd.set('%s',0.5)"%setting_name))

      self.menuBar.addmenuitem(name,  'command', '60%',
                               label='60%',
                               command = lambda s=self: s.cmd.do("_ cmd.set('%s',0.6)"%setting_name))

      self.menuBar.addmenuitem(name,  'command', '80%',
                               label='80%',
                               command = lambda s=self: s.cmd.do("_ cmd.set('%s',0.8)"%setting_name))
