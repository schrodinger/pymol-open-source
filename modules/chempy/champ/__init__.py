...
class Champ:
    ...
    def match_1v1_b(self, pattern, target):  # boolean
        ...
        as a boolean result
        """
        (e, r) = _champ.match_1v1_b(self._champ, int(pattern), int(target))
        if e:
            raise RuntimeError
        return r
    ...
    def match_1v1_n(self, pattern, target, limit, tag=0):
        ...
        """
        (e, r) = _champ.match_1v1_n(
            self._champ, int(pattern), int(target), int(limit), int(tag)
        )
        if e:
            raise RuntimeError
        return r
    ...
    def match_1vN_n(self, pattern, handle):
        ...
        """
        (e, r) = _champ.match_1vN_n(self._champ, int(pattern), int(handle))
        if e:
            raise RuntimeError
        return r
    ...
    def exact_1vN_n(self, pattern, handle):
        ...
        """
        (e, r) = _champ.exact_1vN_n(self._champ, int(pattern), int(handle))
        if e:
            raise RuntimeError
        return r
