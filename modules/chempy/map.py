...
class Map:
    ...
    def from_c_object(self, cobj, format, cell_dim, cell_ang, cell_div, first, last):
        ...
        self.c_object = cobj
        self.cell_ang = [float(cell_ang[0]), float(cell_ang[1]), float(cell_ang[2])]
        self.cell_dim = [float(cell_dim[0]), float(cell_dim[1]), float(cell_dim[2])]
        self.cell_div = [int(cell_div[0]), int(cell_div[1]), int(cell_div[2])]
        self.first = [int(first[0]), int(first[1]), int(first[2])]
        self.last = [int(last[0]), int(last[1]), int(last[2])]
