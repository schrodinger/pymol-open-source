...
class State:
    ...
    def analyze(self, kw=None, summary=1):
        ...
        io.xyz.toFile(self.model, self.prefix + "_inp.xyz", mapping=self.mapping)
            ...
            while 1:
                ...
                if not flag:
                    if lin[0:25] == " Total Potential Energy :":
                        self.summary.append([lin[0:23].strip(), float(lin[25:49])])
                        flag = 1
                else:
                    tok = lin.split()
                    if len(tok):
                        if tok[0] != "Energy":
                            self.summary.append(
                                [lin[0:23].strip(), float(lin[25:49]), int(lin[49:64])]
                            )
            f.close()
        else:
            self.summary = None
    ...
    def dynamics(
        ...
        tinker.do(
            ...
            self.prefix + "_run",
            self.prefix + "_out",
            [tinker.prefix, str(steps), str(timestep), str(interval), str(temperature)],
            capture=1 + self.echo,
        )
