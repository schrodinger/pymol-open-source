...
class CIFRec(CIFData):
    ...
    def read_chem_comp_atom_model_cartn(self, fields, field_dict, values):
        ...
        if symbol is not None:
            str_fields.append(("symbol", symbol))
        if name is not None:
            str_fields.append(("name", name))
        if resn is not None:
            str_fields.append(("resn", resn))
        float_fields = []
    ...
    def read_atom_site_cartn(self, fields, field_dict, values):
        ...
        if auth_resn is not None:
            resn = auth_resn
        if auth_resi is not None:
            resi = auth_resi
        ...
        if resn is not None:
            str_fields.append(("resn", resn))
        if resi is not None:
            str_fields.append(("resi", resi))
        if chain is not None:
            str_fields.append(("chain", chain))
            if ASYM_ID_AS_SEGI:
                str_fields.append(("segi", chain))
        if alt is not None:
            str_fields.append(("alt", alt))
        if ins_code is not None:
            str_fields.append(("ins_code", ins_code))
        float_fields = []
        if q is not None:
            float_fields.append(("q", q))
        if b is not None:
            float_fields.append(("b", b))
        int_fields = []
        if ID is not None:
            int_fields.append(("id", ID))
    ...
    def read_atom_site(self):
        for loop in self.loops:
            if self.read_atom_site_fract(
                None, loop.keys, loop.rows
            ) or self.read_atom_site_cartn(None, loop.keys, loop.rows):
                for loop in self.loops:
    ...
    def read_struct_conn_(self, loop):
        try:
            ...
            asym_id_1 = loop.get_col_idx(
                "_struct_conn.ptnr1_auth_asym_id", "_struct_conn.ptnr1_label_asym_id"
            )
            comp_id_1 = loop.get_col_idx(
                "_struct_conn.ptnr1_auth_comp_id", "_struct_conn.ptnr1_label_comp_id"
            )
            seq_id_1 = loop.get_col_idx(
                "_struct_conn.ptnr1_auth_seq_id", "_struct_conn.ptnr1_label_seq_id"
            )
            atom_id_1 = loop.get_col_idx("_struct_conn.ptnr1_label_atom_id")

            asym_id_2 = loop.get_col_idx(
                "_struct_conn.ptnr2_auth_asym_id", "_struct_conn.ptnr2_label_asym_id"
            )
            comp_id_2 = loop.get_col_idx(
                "_struct_conn.ptnr2_auth_comp_id", "_struct_conn.ptnr2_label_comp_id"
            )
            seq_id_2 = loop.get_col_idx(
        ...
        atom_dict = dict(
            ((a.chain, a.resn, a.resi, getattr(a, "ins_code", ""), a.name, a.alt), i)
    ...
    def read_ss_(self, loop, ss, ssrecords):
        ...
        try:
            Beg_NDB_Strand_ID = loop.get_col_idx(
                prefix + ".beg_auth_asym_id", prefix + ".beg_label_asym_id"
            )
            Beg_NDB_res_num = loop.get_col_idx(
                prefix + ".beg_auth_seq_id", prefix + ".beg_label_seq_id"
            )
            End_NDB_Strand_ID = loop.get_col_idx(
                prefix + ".end_auth_asym_id", prefix + ".end_label_asym_id"
            )
            End_NDB_res_num = loop.get_col_idx(
                prefix + ".end_auth_seq_id", prefix + ".end_label_seq_id"
            )
        except KeyError:
            return False
    ...
    def read_ss(self):
        ...
        for i, a in enumerate(atoms):
            ...
            try:
                ss, endkey = ssrecords[a.chain, a.resi, getattr(a, "ins_code", "")]
            except KeyError:
                continue
