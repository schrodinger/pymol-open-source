...
def do(input, run_prefix=None, echo=None, punch=None, output=None, skip=None):
    if not run_prefix:
        run_prefix = "gamess_run"
    if not skip:
        if feedback["gamess"]:
            print(
                " " + str(__name__) + ': creating temporary files "%s.*"' % (run_prefix)
            )
        ...
        if echo:
            os.system(
                rungms_path + " " + run_prefix + " 2>&1 | tee " + run_prefix + ".out"
            )
        else:
...
class State:
    ...
    def get_data_group(self, basis=None, zmat=1):
        ...
        for z in self.get_zmat_ordering():
            ...
            if not len(a.name):
                ...
                name = a.name
            gmsList.append(
                "%10s %5.1f %18.10f %18.10f %18.10f\n"
                % (name, atNum[a.symbol], a.coord[0], a.coord[1], a.coord[2])
            )
            c = c + 1
        gmsList.append(" $END\n")
        return gmsList
    ...
    def get_contrl_group(
        self, scftyp="RHF", runtyp="ENERGY", exetyp="RUN", coord="UNIQUE", nzvar=-1
    ):
        gmsList = []
        model = self.model
        if nzvar:
        ...
        gmsList.append(
            " $CONTRL SCFTYP=%s RUNTYP=%s EXETYP=%s\n" % (scftyp, runtyp, exetyp)
        )
        if coord:
    ...
    def read_output_list(self, list):
        ...
        if len(crd_list):
            ...
            if cc and feedback["gamess"]:
                print(
                    " " + str(__name__) + ": coordinates modified for %d atoms." % (cc)
                )
        if len(chg_list):
            a = chg_list.pop()
            cc = 0
        ...
        if len(nrg_list):
            ...
            if feedback["gamess"]:
                print(
                    " "
                    + str(__name__)
                    + ": energy updated %12.6f." % self.model.molecule.energy
                )
        ...
        if len(den_list):
            ...
            if feedback["gamess"]:
                print(
                    " "
                    + str(__name__)
                    + ": read density slice %d of %d." % (z_step + 1, brick.dim[2])
                )
        ...
        if len(pot_list):
            ...
            if feedback["gamess"]:
                print(
                    " "
                    + str(__name__)
                    + ": read potential slice %d of %d." % (z_step + 1, brick.dim[2])
                )

    def get_basis_group(self, gbasis="N31", ngauss=6, ndfunc=1):
        gmsList = []
        gmsList.append(
            " $BASIS GBASIS=%s NGAUSS=%d NDFUNC=%d\n" % (gbasis, ngauss, ndfunc)
        )
        model = self.model
        chg = 0
        for a in model.atom:
    ...
    def get_zmat_ext_freeze_torsion(self, flag=3):
        ...
        for a in list(topo.torsion.keys()):
            if (
                model.atom[a[0]].flags
                & model.atom[a[1]].flags
                & model.atom[a[2]].flags
                & model.atom[a[3]].flags
            ) & mask:
        ...
        if len(frozen_list):
            ...
            for frozen in frozen_list:
                ...
                dihe = cmd.get_dihedral(
                    "(_gamess1 and id %d)" % fixed[0],
                ...
                print(
                    " freeze-torsion: %d-%d-%d-%d (pymol), %d-%d-%d-%d (gamess)"
                    % (
                        fixed[0],
                        fixed[1],
                        fixed[2],
    ...
    def get_prop_job(self):
        gmsList = []
        gmsList.extend(self.get_contrl_group(runtyp="PROP", nzvar=0))
    ...
    def get_energy_job(self):
        gmsList = []
        gmsList.extend(self.get_contrl_group(runtyp="ENERGY"))
        gmsList.extend(self.get_basis_group())
    ...
    def get_density(self, brick, morb=0, run_prefix=None):
        for a in range(brick.dim[2]):
            ...
            result = do(gmsList, punch=1, run_prefix=run_prefix)
    ...
    def get_charges(self, run_prefix=None):
        gmsList = self.get_energy_job()
        result = do(gmsList, output=1, punch=1, run_prefix=run_prefix)
    ...
    def get_optimized_charges(self, run_prefix=None, skip=None):
        ...
        result = do(gmsList, output=1, punch=1, run_prefix=run_prefix, skip=skip)
        self.read_output_list(result[0])
        self.read_punch_list(result[1])
