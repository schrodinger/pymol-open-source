    ...
    def toList(self, model, **kwargs):
        ...
        for atom_id, at in enumerate(model.atom, 1):
            ...
            if not (at_prev and at_prev.in_same_residue(at)):
                residues.append(
                    [subst_name, atom_id, at.chain or at.segi or "****", resn, at.flags]
                )
                at_prev = at
        ...
        if no_text_type_count > 0:
            print(
                " Warning: %d atoms missing 'text_type', using element symbol instead."
        ...
        for b, bo in enumerate(model.bond):
            ...
            buf.append("%d %d %d %s\n" % (b, 1 + bo.index[0], 1 + bo.index[1], bOrder))
