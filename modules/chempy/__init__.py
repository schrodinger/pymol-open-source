...
class Atom(object):
    ...
    def new_in_residue(self):
        newat = Atom()
        if self.has("segi"):
            newat.segi = self.segi
        if self.has("chain"):
            newat.chain = self.chain
        if self.has("resn"):
            newat.resn = self.resn
        if self.has("resn_code"):
            newat.resn_code = self.resn_code
        if self.has("ins_code"):
            newat.ins_code = self.ins_code
        if self.has("resi_number"):
            newat.resi_number = self.resi_number
        if self.has("hetatm"):
            newat.hetatm = self.hetatm
        return newat

    def get_signature(self):
        return ":".join(
            [self.segi, self.chain, self.resn, self.resi, self.symbol, self.name]
        )

    def __cmp__(self, other):
        return (
            cmp(type(self), type(other))
            or cmp(self.segi, other.segi)
            or cmp(self.chain, other.chain)
            or cmp(self.resi_number, other.resi_number)
            or cmp(self.ins_code, other.ins_code)
            or cmp(self.resn, other.resn)
