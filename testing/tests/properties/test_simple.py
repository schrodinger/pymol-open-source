...
class TestProperties(testing.PyMOLTestCase):
    ...
    def testMAEloadSomeProperties(self):
        ...
        cmd.load(
            self.datafile("1d_smiles.mae"), "1d_smiles", object_props=" ".join(props)
        )
        objs = cmd.get_object_list()
        for obj in objs:
        ...
        for obj in objs:
            ...
            for prop in props:
                try:
                    ...
                    self.fail(
                        "properties are not the same: obj=%s prop=%s" % (obj, prop)
                    )
    ...
    def testSDF(self):
        ...
        self.assertEqual(
            None,
            cmd.get_property("PUBCHEM_COMPOUND_CID", "m1"),
