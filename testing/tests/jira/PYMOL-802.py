...
class TestPYMOL802(testing.PyMOLTestCase):
    ...
    @testing.foreach(
        (-1, "on", True), (1, "on", True), (0, "on", False), (-1, "off", False)
    )
    def test802(self, ray_opaque_background, opaque_background, trans_check):
        ...
        if trans_check:
            self.assertImageHasNoTransparency(
                self.get_imagearray(width=100, height=100, ray=1)
            )
        else:
            self.assertImageHasTransparency(
                self.get_imagearray(width=100, height=100, ray=1)
            )
