...
def test_undo_select_merge():
    ...
    _undo_assert_selections("?foo", NC - 1, ["foo"], ["foo"], NC + 1, ["foo"], ["foo"])
    ...
    cmd.select("present")
    _undo_assert_selections("present", 7, ["foo"], [], 7, ["foo", "sele"], ["sele"])
    ...
    _undo_assert_selections(
        "present", 2, ["foo"], [], 2, ["foo", "sele"], ["sele"], state=2
    )

    cmd.delete("sele")
    cmd.select("present", state=3)
    _undo_assert_selections(
        "present", 0, ["foo"], [], 0, ["foo", "sele"], ["sele"], state=3
    )
    ...
    _undo_assert_selections("?bar", 0, ["foo"], ["foo"], 1, ["foo", "bar"], ["bar"])
