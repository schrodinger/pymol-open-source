...
class TestSideChainHelper(testing.PyMOLTestCase):
    ...
    @testing.foreach.product(
        ...
        [
            "cartoon",
            "ribbon",
        ],  # for setting X_side_chain_helper and X_nucleic_acid_mode
        [0, 1],
    )  # X_nucleic_acid_mode
    ...
    def test(self, bb_rep, sc_rep, bb_set, n_a_m):
        ...
        for bb_test in ["cartoon", "ribbon"]:
            self._assertImageHasColor(
                bb_rep == bb_test, color[bb_test], img, 0, bb_test + " wrong"
            )
        self._assertImageHasColor(not (bb_rep == bb_set), op_color, img, 0, "OP wrong")
        self._assertImageHasColor(
            not (bb_rep == bb_set and n_a_m == 1), p_color, img, 0, "NAM=1 wrong"
        )
