...
class TestPyMOL2(testing.PyMOLTestCase):
    ...
    @testing.requires_version("2.1" if invocation.options.incentive_product else "1.9")
    def testDel(self):
        import pymol2
        import weakref
    ...
    def _test_colortype(self, _self):
        ...
        _self.iterate(
            "*",
            "colorset.add((" "tuple(s.sphere_color)," "int(s.sphere_color)))",
            space={"colorset": colorset, "tuple": tuple, "int": int},
        )
        self.assertEqual(list(colorset), [((0.0, 0.0, 1.0), 2)])
