...
class TestPreset(testing.PyMOLTestCase):
    ...
    def _assertSimpleReps(self):
        ...
        self.assertEqual(
            cmd.count_atoms("rep ribbon & guide"), cmd.count_atoms("guide")
        )
        self.assertEqual(
            cmd.count_atoms("rep sticks & organic"), cmd.count_atoms("organic")
        )
        self.assertEqual(
            cmd.count_atoms("rep nonbonded & solvent"), cmd.count_atoms("solvent")
        )

    def _assertPrettyReps(self):
        self.assertEqual(
            cmd.count_atoms("rep cartoon & guide"), cmd.count_atoms("guide")
        )
