...
class TestSetting(testing.PyMOLTestCase):
    ...
    def testGetSettingFloatExact(self):
        ...
        for v_ref in (1.234567, 1234567.0e6, 9999999.0):
    ...
    def testUnsetDeep(self):
        ...
        # object-level
        cmd.set("sphere_scale", 0.5, "ala")
        cmd.set("stick_radius", 0.4, "gly")

        # atom-level
        cmd.set("sphere_scale", 0.3, "index 1-3")
        cmd.set("sphere_color", "yellow", "index 2-4")

        # bond-level
        cmd.set("stick_radius", 0.6, "elem C")
        ...
        for oname in names:
            ...
            self.assertAlmostEqual(
                sphere_scale_global, list(a_level_values)[0], delta=1e-4
            )
        ...
        for sname in ["stick_radius", "stick_color"]:
            ...
            for o_set in b_level_values:
                for b_set in o_set[1]:
                    self.assertEqual(
                        None, b_set[2], msg=sname + " " + o_set[0] + " " + str(b_set)
                    )
