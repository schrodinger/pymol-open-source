...
class TestExporting(testing.PyMOLTestCase):
    ...
    def testMultifilesave(self):
        ...
        for fmt in ["{}-{:02}.cif", "{}-{state}.cif"]:
            with testing.mkdtemp() as dirname:
                ...
                filenames = [
                    os.path.basename(p)
                    for p in glob.glob(os.path.join(dirname, "*.cif"))
                ]
    ...
    def testSaveRef(self, format):
        ...
        with testing.mktemp("." + format) as filename:
            ...
            self.assertAlmostEqual(
                cmd.rms_cur("m4", "m1", matchmaker=m), 0.00, delta=1e-2
            )
            self.assertAlmostEqual(
                cmd.rms_cur("m4", "m2", matchmaker=m), 1.87, delta=1e-2
            )
        ...
        with testing.mktemp("." + format) as filename:
            ...
            self.assertAlmostEqual(
                cmd.rms_cur("m5", "m2", matchmaker=m), 0.00, delta=1e-2
            )
            self.assertAlmostEqual(
                cmd.rms_cur("m5", "m1", matchmaker=m), 1.87, delta=1e-2
            )
    ...
    def testSaveState(self, format, pymol_version):
        ...
        with testing.mktemp("." + format) as filename:
            ...
            for state in range(1, n_states + 1):
                rms = cmd.rms_cur(
                    "m1", "m2 and state %d" % state, state, state, matchmaker=m
                )
                self.assertAlmostEqual(rms, 0.00, delta=1e-2)
    ...
    def testSaveSelection(self, format, pymol_version):
        ...
        with testing.mktemp("." + format) as filename:
            if testing.PYMOL_VERSION_TUPLE < (1, 8):
                cmd.set(
                    "raise_exceptions", 0
                )  # 1.7.6 save xyz doesn't set r=DEFAULT_SUCCESS
            ...
            cmd.delete("*")
            cmd.load(
                filename, "m2", discrete=1, format=format
            )  # avoid merging of atoms
    ...
    def cmp_atom(self, selfobj, other):
        ...
        return (
            cmp(type(selfobj), type(other))
            or cmp(selfobj.segi, other.segi)
            or cmp(selfobj.chain, other.chain)
            or cmp(selfobj.resi_number, other.resi_number)
            or cmp(selfobj.resi, other.resi)
            or cmp(selfobj.resn, other.resn)
            or cmp(selfobj.symbol, other.symbol)
            or cmp(selfobj.name, other.name)
        )

    # cmp_bond : compares all fields in Bond (see chempy/__init__.py)
    def cmp_bond(self, selfobj, other):
        cmp = lambda a, b: a != b  # py3k
        return cmp(selfobj.order, other.order) or cmp(selfobj.index, other.index)
