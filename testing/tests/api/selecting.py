...
class TestSelecting(testing.PyMOLTestCase):
    ...
    def testMacros(self):
        ...
        self.assertEqual(cmd.count_atoms("////`100"), cmd.count_atoms("resi 100"))
        self.assertEqual(
            cmd.count_atoms("////100-110"), cmd.count_atoms("resi 100-110")
        )
        self.assertEqual(cmd.count_atoms("////ARG`100"), cmd.count_atoms("resi 100"))
        self.assertEqual(
            cmd.count_atoms("////ALA"),
            cmd.count_atoms("resn ALA"),
        )
        self.assertEqual(
            cmd.count_atoms("ALA/"),
            cmd.count_atoms("resn ALA"),
        ...
        self.assertEqual(
            cmd.count_atoms("////ALA+GLU"),
            cmd.count_atoms("resn ALA+GLU"),
        )
        ...
        self.assertEqual(cmd.count_atoms("*/CG"), cmd.count_atoms("name CG"))
        self.assertEqual(
            cmd.count_atoms("/////CG`B"), cmd.count_atoms("name CG & alt B")
        )
        self.assertEqual(cmd.count_atoms("/////`B"), cmd.count_atoms("alt B"))

        # combies
        self.assertEqual(
            cmd.count_atoms("100/CA"), cmd.count_atoms("resi 100 & name CA")
        )
        ...
        self.assertEqual(
            cmd.count_atoms("/m1/Y/B/*/CA"),
            cmd.count_atoms("segi Y & chain B & name CA"),
        )
    ...
    def test_no_all_dummy_selection(self):
        ...
        self.assertEqual(
            cmd.count_atoms("all within 1.5 of (center within 1.5 of p1)"), 2
        )
        self.assertEqual(cmd.count_atoms("all near_to 1.5 of center"), 2)
        self.assertEqual(
            cmd.count_atoms("all near_to 1.5 of (all near_to 1.5 of p1)"), 0
        )
        self.assertEqual(
            cmd.count_atoms("all near_to 1.5 of (center near_to 1.5 of p1)"), 2
        )
