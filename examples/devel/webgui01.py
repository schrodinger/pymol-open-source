...
def get_status(out, self_cmd=cmd):
    ...
    out.write("<table><tr>\n")
    out.write(
        '<td><form action="./status.pymol"><button type="submit">Refresh</button></form></td>\n'
    )
    out.write(
        '<td><form target="_new" action="./ray.pymol?t=%f"><button type="submit">Ray</button></form></td>\n'
        % time.time()
    )
    out.write(
        '<td><form target="_new" action="./monitor.pymol?t=%f"><button type="submit">Monitor</button></form></td>\n'
        % time.time()
    )
    out.write(
        '<td><form action="./quit.pymol"><button type="submit">Quit</button></form></td>\n'
    )
    out.write("</tr></table>")
    ...
    if not len(names):
        ...
        out.write("</ul>\n")
    out.write(
        '<a href="#" onClick="updateImage()"><img src="./draw.pymol?t=%f">'
        % time.time()
        + "</img></a>"
    )
    out.write("</body></html>\n")
