...
class PickWizard(Wizard):
    ...
    def get_prompt(self):
        ...
        if self.pk2_st != None:
            return [
                "You picked the bond between %s and %s" % (self.pk1_st, self.pk2_st)
            ]
        elif self.pk1_st != None:
            return [
                "You picked atom %s" % (self.pk1_st),
    ...
    def do_pick(self, picked_bond):
        ...
        cmd.iterate(
            "pk1",
            "setattr(cmd.get_wizard(),'pk1_st',"
            "'%s/%s/%s/%s/%s'%(model,segi,chain,resi,name))",
        )
        if picked_bond:
            cmd.iterate(
                "pk1",
                "setattr(cmd.get_wizard(),'pk2_st',"
                "'%s/%s/%s/%s/%s'%(model,segi,chain,resi,name))",
            )
        else:
            ...
            cmd.iterate_state(
                cmd.get_state(), "pk1", "setattr(cmd.get_wizard(),'pk1_xyz',(x,y,z))"
            )
